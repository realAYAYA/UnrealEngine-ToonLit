...
struct FCapture
{
	...
	void Run()
	{
		...
		while (true)
		{
			...
			if (acStatus.IsRunning() && acStatus.HasAvailableInputFrame())
			{
				...
				const bool bUseLTCTimecode = true;
				NTV2_RP188	timecode;
				inputXfer.GetInputTimeCode(timecode, NTV2ChannelToTimecodeIndex(mChannel, bUseLTCTimecode));
				...
				FrameRP188Info.SetRP188(timecode, TimecodeFormat);
				...
				TCBurner.BurnTimeCode((char*)pVideoBuffer, TimeCodeString.c_str(), 40);

				//if (TimeCodeString.empty())
				//{
				//	std::cerr << "aslkjasdjkl;sdfajlk;afsd";
				//}
				...
				//if (Counter%10 == 0)
				//	::Sleep(200);
				Bitmap::drawbmp(ss.str().c_str(), pVideoBuffer, FormatDesc.GetRasterWidth(), FormatDesc.GetRasterHeight(), FormatDesc.GetBytesPerRow());


				//https://en.wikipedia.org/wiki/User:Evercat/Buddhabrot.c
				//void drawbmp(const char* filename, uint8_t* videoBuffer, int32_t width, int32_t height, uint32_t pitch)

				//FILE* outfile = nullptr;
				//errno_t erro = fopen_s(&outfile, filename, "wb");
				//if (erro != 0)
				//	return;
				...
				//for (int32_t y = height - 1; y >= 0; y--)     // BMP image format is written from bottom to top...
				//{
				...
				//		//ARGB
				//		uint32_t color = *(uint32_t*)(videoBuffer + y*pitch + x * 4);
				//		uint8_t alpha = color & 0xFF000000;
	...
	bool AskQuestions()
	{
		...
		{
			std::wcout << "Is the incoming pixel format RGB?" << std::endl;
