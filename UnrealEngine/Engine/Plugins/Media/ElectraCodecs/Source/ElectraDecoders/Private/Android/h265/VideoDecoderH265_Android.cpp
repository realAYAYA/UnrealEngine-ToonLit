...
private:
	...
	TSharedPtr<ElectraDecodersUtil::MPEG::FHEVCDecoderConfigurationRecord, ESPMode::ThreadSafe> ConfigRecord;
...
TSharedPtr<ElectraDecodersUtil::MPEG::FHEVCDecoderConfigurationRecord, ESPMode::ThreadSafe> FElectraVideoDecoderH265_Android::GetDCRFromOptions(const TMap<FString, FVariant>& InOptions)
{
...
TSharedPtr<ElectraDecodersUtil::MPEG::FISO23008_2_seq_parameter_set_data, ESPMode::ThreadSafe> FElectraVideoDecoderH265_Android::GetSPSFromOptions(const TMap<FString, FVariant>& InOptions)
{
...
bool FElectraVideoDecoderH265_Android::InternalFinishDecoderCreate()
{
	if (CurrentSurfaceRequest.IsValid())
	{
		...
		if (ConfigRecord.IsValid())
		{
			...
			const ElectraDecodersUtil::MPEG::FISO23008_2_seq_parameter_set_data& SPS = ConfigRecord->GetParsedSPS(0);
			if (SPS.GetWidth() > Width || SPS.GetHeight() > Height || SPS.general_profile_idc > ProfileIdc || SPS.general_level_idc > LevelIdc)
			{
				Width = SPS.GetWidth();
				Height = SPS.GetHeight();
				ProfileIdc = SPS.general_profile_idc;
				LevelIdc = SPS.general_level_idc;
			}
		}
