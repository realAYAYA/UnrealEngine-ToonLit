...
class IElectraVPxVideoDecoderAndroidJava
{
public:

	static TSharedPtr<IElectraVPxVideoDecoderAndroidJava, ESPMode::ThreadSafe> Create();

	virtual ~IElectraVPxVideoDecoderAndroidJava() = default;
	...
	struct FCreateParameters
	{
		...
		const ElectraDecodersUtil::VPxVideo::FVP9UncompressedHeader* CodecData = nullptr;
	 ...
	 */
	virtual int32 CreateDecoder(bool bCreateVP9) = 0;
	 ...
	 * @return 0 if successful, 1 on error.
	 */
	virtual int32 QueueInputBuffer(int32 InBufferIndex, const void* InPrefixData, int32 InPrefixSize, const void* InAccessUnitData, int32 InAccessUnitSize, int64 InTimestampUSec) = 0;
