...
*/
	...
	public int CreateDecoder()
	{
		...
		try
		{
			if (PlatformCodecInfo != null)
			{
				...
				{
					Log.w(TAG, "ElectraDecoderVideoH265: Failed to create decoder by name");
					return 1;
				}
			}
			else
			{
				// No decoder found.
				Log.w(TAG, "ElectraDecoderVideoH265: No suitable decoder found");
				return 1;
			}
		}
		catch(Exception e)
		{
			Log.w(TAG, "ElectraDecoderVideoH265: Failed to create decoder");
			e.printStackTrace();
			return 1;
		}
		return 0;
	...
	public int ConfigureDecoder(FCreateParameters InCreateParams)
	{
		...
		{
			Log.w(TAG, "ElectraDecoderVideoH265: No decoder instance to configure has been created yet");
			return 1;
		}
...
//			Format.setInteger("low-latency", 1);
		...
		{
			Log.w(TAG, "ElectraDecoderVideoH265: Failed to configure the decoder");
			e.printStackTrace();
			return 1;
		}
		return 0;
	...
	public int SetOutputSurface(android.view.Surface NewVideoCodecSurface)
	{
		if (Build.VERSION.SDK_INT >= 23 && bIsInitialized && NewVideoCodecSurface != null)
		{
			...
			{
				Log.w(TAG, "ElectraDecoderVideoH265: Failed to set decoder output surface");
				e.printStackTrace();
				return 1;
			}
			return 0;
	...
	public int ReleaseDecoder()
	{
		if (bIsInitialized)
		{
			...
			{
				Log.w(TAG, "ElectraDecoderVideoH265: Failed to release decoder");
				e.printStackTrace();
				return 1;
			}
			return 0;
	...
	public int Start()
	{
		if (bIsInitialized)
		{
			...
			{
				Log.w(TAG, "ElectraDecoderVideoH265: Failed to start decoder");
				e.printStackTrace();
				return 1;
			}
			return 0;
	...
	public int Stop()
	{
		if (bIsInitialized)
		{
			...
			{
				Log.w(TAG, "ElectraDecoderVideoH265: Failed to stop decoder");
				e.printStackTrace();
				return 1;
			}
			return 0;
	...
	public int Flush()
	{
		if (bIsInitialized)
		{
			...
			{
				Log.w(TAG, "ElectraDecoderVideoH265: Failed to flush decoder");
				e.printStackTrace();
				return 1;
			}
	...
	public int Reset()
	{
		if (bIsInitialized)
		{
			...
			{
				Log.w(TAG, "ElectraDecoderVideoH265: Failed to reset decoder");
				e.printStackTrace();
				return 1;
			}
			return 0;
	...
	public int DequeueInputBuffer(int InTimeoutUsec)
	{
		if (bIsInitialized)
		{
			...
			{
				Log.w(TAG, "ElectraDecoderVideoH265: Failed to dequeue input buffer");
				e.printStackTrace();
				return -10000;
			}
			return BufferIndex;
	...
	public int QueueInputBuffer(int InBufferIndex, long InTimestampUSec, byte[] InAccessUnit)
	{
		if (bIsInitialized)
		{
			if (InBufferIndex >= 0)
			{
				...
				{
					Log.w(TAG, "ElectraDecoderVideoH265: Failed to queue input buffer");
					e.printStackTrace();
					return -10000;
				}
				return 0;
	...
	public int QueueCSDInputBuffer(int InBufferIndex, long InTimestampUSec, byte[] InCSD)
	{
		if (bIsInitialized)
		{
			if (InBufferIndex >= 0)
			{
				...
				{
					Log.w(TAG, "ElectraDecoderVideoH265: Failed to queue CSD input buffer");
					e.printStackTrace();
					return -10000;
				}
				return 0;
	...
	public int QueueEOSInputBuffer(int InBufferIndex, long InTimestampUSec)
	{
		if (bIsInitialized)
		{
			if (InBufferIndex >= 0)
			{
				...
				{
					Log.w(TAG, "ElectraDecoderVideoH265: Failed to queue EOS input buffer");
					e.printStackTrace();
					return -10000;
				}
				return 0;
	...
	public FOutputBufferInfo DequeueOutputBuffer(int InTimeoutUsec)
	{
		if (bIsInitialized)
		{
			...
			{
				Log.w(TAG, "ElectraDecoderVideoH265: Failed to dequeue output buffer");
				e.printStackTrace();
				return null;
			}
	...
	public byte[] GetOutputBuffer(int InOutputBufferIndex)
	{
		...
		if (bIsInitialized && InOutputBufferIndex >= 0)
		{
			...
			if (Info != null)
			{
				...
				{
					Log.w(TAG, "ElectraDecoderVideoH265: Failed to get output buffer");
					e.printStackTrace();
					return null;
				}
			}
	...
	public int ReleaseOutputBuffer(int InOutputBufferIndex, boolean bRender, long releaseAt)
	{
		if (bIsInitialized && InOutputBufferIndex >= 0)
		{
			...
			if (Info != null)
			{
				...
				{
					Log.w(TAG, "ElectraDecoderVideoH265: Failed to release buffer to surface");
					e.printStackTrace();
					return 1;
				}
				return 0;
