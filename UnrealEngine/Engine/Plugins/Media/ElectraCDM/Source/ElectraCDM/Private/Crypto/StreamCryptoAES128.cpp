...
{
public:
	void SetKey(const uint8* InKey)
	{
		uint32 *RK = RoundKey;
		RK[0] = GetUInt32(InKey);
		RK[1] = GetUInt32(InKey + 4);
		RK[2] = GetUInt32(InKey + 8);
		RK[3] = GetUInt32(InKey + 12);
		for(int32 i=0; i<10; ++i)
		{
			uint32 temp = RK[3];
			RK[4] = RK[0] ^ (T2[(temp >> 16) & 255] & 0xff000000) ^ (T3[(temp >> 8) & 255] & 0xff0000) ^ (T0[temp & 255] & 0xff00) ^ (T1[temp >> 24] & 0xff) ^ RoundConstant[i];
			RK[5] = RK[1] ^ RK[4];
			RK[6] = RK[2] ^ RK[5];
			RK[7] = RK[3] ^ RK[6];
			RK += 4;
	...
	void SetIV(const TArray<uint8>& InIV)
	{
		if (InIV.Num() <= 16)
		{
			FMemory::Memzero(IV, 16);
			FMemory::Memcpy(IV, InIV.GetData(), InIV.Num());
		}
		CipherPos = 0;
	}
	...
	void Decrypt(uint8* InOutData, int32 InNumBytes)
	{
		while(InNumBytes)
		{
			if (CipherPos == 0)
			{
				Cipher(CurrentCipherBuffer, IV);
			}
			int32 maxb = 16 - CipherPos;
			int32 n = InNumBytes < maxb ? InNumBytes : maxb;
			for(int32 i=0; i<n; ++i, ++InOutData)
			{
				*InOutData = *InOutData ^ CurrentCipherBuffer[CipherPos];
				if (++CipherPos == 16)
				{
					CipherPos = 0;
					IncrementCounter();
...
private:
	void Cipher(uint8* Out, const uint8* In) const
	{
		uint32 s0, s1, s2, s3, t0, t1, t2, t3;
		s0 = GetUInt32(In)      ^ RoundKey[0];
		s1 = GetUInt32(In + 4)  ^ RoundKey[1];
		s2 = GetUInt32(In + 8)  ^ RoundKey[2];
		s3 = GetUInt32(In + 12) ^ RoundKey[3];
		t0 = T0[s0 >> 24] ^ T1[(s1 >> 16) & 255] ^ T2[(s2 >> 8) & 255] ^ T3[s3 & 255] ^ RoundKey[ 4];
		t1 = T0[s1 >> 24] ^ T1[(s2 >> 16) & 255] ^ T2[(s3 >> 8) & 255] ^ T3[s0 & 255] ^ RoundKey[ 5];
		t2 = T0[s2 >> 24] ^ T1[(s3 >> 16) & 255] ^ T2[(s0 >> 8) & 255] ^ T3[s1 & 255] ^ RoundKey[ 6];
		t3 = T0[s3 >> 24] ^ T1[(s0 >> 16) & 255] ^ T2[(s1 >> 8) & 255] ^ T3[s2 & 255] ^ RoundKey[ 7];
		s0 = T0[t0 >> 24] ^ T1[(t1 >> 16) & 255] ^ T2[(t2 >> 8) & 255] ^ T3[t3 & 255] ^ RoundKey[ 8];
		s1 = T0[t1 >> 24] ^ T1[(t2 >> 16) & 255] ^ T2[(t3 >> 8) & 255] ^ T3[t0 & 255] ^ RoundKey[ 9];
		s2 = T0[t2 >> 24] ^ T1[(t3 >> 16) & 255] ^ T2[(t0 >> 8) & 255] ^ T3[t1 & 255] ^ RoundKey[10];
		s3 = T0[t3 >> 24] ^ T1[(t0 >> 16) & 255] ^ T2[(t1 >> 8) & 255] ^ T3[t2 & 255] ^ RoundKey[11];
		t0 = T0[s0 >> 24] ^ T1[(s1 >> 16) & 255] ^ T2[(s2 >> 8) & 255] ^ T3[s3 & 255] ^ RoundKey[12];
		t1 = T0[s1 >> 24] ^ T1[(s2 >> 16) & 255] ^ T2[(s3 >> 8) & 255] ^ T3[s0 & 255] ^ RoundKey[13];
		t2 = T0[s2 >> 24] ^ T1[(s3 >> 16) & 255] ^ T2[(s0 >> 8) & 255] ^ T3[s1 & 255] ^ RoundKey[14];
		t3 = T0[s3 >> 24] ^ T1[(s0 >> 16) & 255] ^ T2[(s1 >> 8) & 255] ^ T3[s2 & 255] ^ RoundKey[15];
		s0 = T0[t0 >> 24] ^ T1[(t1 >> 16) & 255] ^ T2[(t2 >> 8) & 255] ^ T3[t3 & 255] ^ RoundKey[16];
		s1 = T0[t1 >> 24] ^ T1[(t2 >> 16) & 255] ^ T2[(t3 >> 8) & 255] ^ T3[t0 & 255] ^ RoundKey[17];
		s2 = T0[t2 >> 24] ^ T1[(t3 >> 16) & 255] ^ T2[(t0 >> 8) & 255] ^ T3[t1 & 255] ^ RoundKey[18];
		s3 = T0[t3 >> 24] ^ T1[(t0 >> 16) & 255] ^ T2[(t1 >> 8) & 255] ^ T3[t2 & 255] ^ RoundKey[19];
		t0 = T0[s0 >> 24] ^ T1[(s1 >> 16) & 255] ^ T2[(s2 >> 8) & 255] ^ T3[s3 & 255] ^ RoundKey[20];
		t1 = T0[s1 >> 24] ^ T1[(s2 >> 16) & 255] ^ T2[(s3 >> 8) & 255] ^ T3[s0 & 255] ^ RoundKey[21];
		t2 = T0[s2 >> 24] ^ T1[(s3 >> 16) & 255] ^ T2[(s0 >> 8) & 255] ^ T3[s1 & 255] ^ RoundKey[22];
		t3 = T0[s3 >> 24] ^ T1[(s0 >> 16) & 255] ^ T2[(s1 >> 8) & 255] ^ T3[s2 & 255] ^ RoundKey[23];
		s0 = T0[t0 >> 24] ^ T1[(t1 >> 16) & 255] ^ T2[(t2 >> 8) & 255] ^ T3[t3 & 255] ^ RoundKey[24];
		s1 = T0[t1 >> 24] ^ T1[(t2 >> 16) & 255] ^ T2[(t3 >> 8) & 255] ^ T3[t0 & 255] ^ RoundKey[25];
		s2 = T0[t2 >> 24] ^ T1[(t3 >> 16) & 255] ^ T2[(t0 >> 8) & 255] ^ T3[t1 & 255] ^ RoundKey[26];
		s3 = T0[t3 >> 24] ^ T1[(t0 >> 16) & 255] ^ T2[(t1 >> 8) & 255] ^ T3[t2 & 255] ^ RoundKey[27];
		t0 = T0[s0 >> 24] ^ T1[(s1 >> 16) & 255] ^ T2[(s2 >> 8) & 255] ^ T3[s3 & 255] ^ RoundKey[28];
		t1 = T0[s1 >> 24] ^ T1[(s2 >> 16) & 255] ^ T2[(s3 >> 8) & 255] ^ T3[s0 & 255] ^ RoundKey[29];
		t2 = T0[s2 >> 24] ^ T1[(s3 >> 16) & 255] ^ T2[(s0 >> 8) & 255] ^ T3[s1 & 255] ^ RoundKey[30];
		t3 = T0[s3 >> 24] ^ T1[(s0 >> 16) & 255] ^ T2[(s1 >> 8) & 255] ^ T3[s2 & 255] ^ RoundKey[31];
		s0 = T0[t0 >> 24] ^ T1[(t1 >> 16) & 255] ^ T2[(t2 >> 8) & 255] ^ T3[t3 & 255] ^ RoundKey[32];
		s1 = T0[t1 >> 24] ^ T1[(t2 >> 16) & 255] ^ T2[(t3 >> 8) & 255] ^ T3[t0 & 255] ^ RoundKey[33];
		s2 = T0[t2 >> 24] ^ T1[(t3 >> 16) & 255] ^ T2[(t0 >> 8) & 255] ^ T3[t1 & 255] ^ RoundKey[34];
		s3 = T0[t3 >> 24] ^ T1[(t0 >> 16) & 255] ^ T2[(t1 >> 8) & 255] ^ T3[t2 & 255] ^ RoundKey[35];
		t0 = T0[s0 >> 24] ^ T1[(s1 >> 16) & 255] ^ T2[(s2 >> 8) & 255] ^ T3[s3 & 255] ^ RoundKey[36];
		t1 = T0[s1 >> 24] ^ T1[(s2 >> 16) & 255] ^ T2[(s3 >> 8) & 255] ^ T3[s0 & 255] ^ RoundKey[37];
		t2 = T0[s2 >> 24] ^ T1[(s3 >> 16) & 255] ^ T2[(s0 >> 8) & 255] ^ T3[s1 & 255] ^ RoundKey[38];
		t3 = T0[s3 >> 24] ^ T1[(s0 >> 16) & 255] ^ T2[(s1 >> 8) & 255] ^ T3[s2 & 255] ^ RoundKey[39];
		s0 = (T2[(t0 >> 24)] & 0xff000000) ^ (T3[(t1 >> 16) & 255] & 0xff0000) ^ (T0[(t2 >> 8) & 255] & 0xff00) ^ (T1[t3 & 255] & 0xff) ^ RoundKey[40];
		s1 = (T2[(t1 >> 24)] & 0xff000000) ^ (T3[(t2 >> 16) & 255] & 0xff0000) ^ (T0[(t3 >> 8) & 255] & 0xff00) ^ (T1[t0 & 255] & 0xff) ^ RoundKey[41];
		s2 = (T2[(t2 >> 24)] & 0xff000000) ^ (T3[(t3 >> 16) & 255] & 0xff0000) ^ (T0[(t0 >> 8) & 255] & 0xff00) ^ (T1[t1 & 255] & 0xff) ^ RoundKey[42];
		s3 = (T2[(t3 >> 24)] & 0xff000000) ^ (T3[(t0 >> 16) & 255] & 0xff0000) ^ (T0[(t1 >> 8) & 255] & 0xff00) ^ (T1[t2 & 255] & 0xff) ^ RoundKey[43];
		PutUInt32(Out,      s0);
		PutUInt32(Out + 4,  s1);
		PutUInt32(Out + 8,  s2);
		PutUInt32(Out + 12, s3);
	...
	static uint32 GetUInt32(const uint8* InFrom)
	{
		return ((uint32)InFrom[0] << 24) | ((uint32)InFrom[1] << 16) | ((uint32)InFrom[2] << 8) | (uint32)InFrom[3];
	}

	static void PutUInt32(uint8* OutTo, uint32 v)
	{
		OutTo[0] = uint8(v >> 24);
		OutTo[1] = uint8(v >> 16);
		OutTo[2] = uint8(v >> 8);
	...
	static const uint32	RoundConstant[10];
	uint32 RoundKey[44];
	uint8 IV[16];
	int32 CipherPos = 0;
	uint8 CurrentCipherBuffer[16];
};
const uint32 FAES128CTR::T0[256] =
{
...
const uint32 FAES128CTR::T1[256] =
{
...
const uint32 FAES128CTR::T2[256] =
{
...
const uint32 FAES128CTR::T3[256] =
{
...
const uint32 FAES128CTR::RoundConstant[10] =
{
...
public:
	...
	virtual EResult CTRInit(const TArray<uint8>& Key) override;
	virtual EResult CTRSetKey(const TArray<uint8>& Key) override;
	virtual EResult CTRSetIV(const TArray<uint8>& IV) override;
	virtual EResult CTRDecryptInPlace(uint8* InOutData, int32 InNumBytes) override;
private:
	TinyAES128::AES_ctx		AES128CBCContext;
	FAES128CTR				AES128CTR;
	TArray<uint8>			CryptoKey;
	bool					bIsInitialized;
};
...
IStreamDecrypterAES128::EResult FStreamDecrypterAES128::CBCInit(const TArray<uint8>& Key, const TArray<uint8>* OptionalIV)
{
	...
	if (Key.Num() == 16)
	{
		...
		if (OptionalIV)
		{
			TinyAES128::AES_init_ctx_iv(&AES128CBCContext, (const uint8_t*)Key.GetData(), (const uint8_t*)OptionalIV->GetData());
		}
		else
		{
...
IStreamDecrypterAES128::EResult FStreamDecrypterAES128::CBCDecryptInPlace(int32& OutNumBytes, uint8* InOutData, int32 NumBytes16, bool bIsFinalBlock)
{
	...
	if (bIsInitialized)
	{
		...
		if ((NumBytes16 & 15) == 0)
		{
			if (NumBytes16)
			{
				TinyAES128::AES_CBC_decrypt_buffer(&AES128CBCContext, (uint8_t*)InOutData, (uint32_t)NumBytes16);
...
IStreamDecrypterAES128::EResult FStreamDecrypterAES128::CBCEncryptInPlace(int32& OutNumBytes, uint8* InOutData, int32 NumBytes, bool bIsFinalData)
{
	...
	if (bIsInitialized)
	{
		...
		if (NumBlockBytes)
		{
			TinyAES128::AES_CBC_encrypt_buffer(&AES128CBCContext, (uint8_t*)InOutData, (uint32_t)NumBlockBytes);
		...
		if (bIsFinalData)
		{
			...
			TinyAES128::AES_CBC_encrypt_buffer(&AES128CBCContext, (uint8_t*)(InOutData + NumBlockBytes), 16);
			OutNumBytes += 16;
		}
		return IStreamDecrypterAES128::EResult::Ok;
	}
...
IStreamDecrypterAES128::EResult FStreamDecrypterAES128::CTRInit(const TArray<uint8>& Key)
{
	bIsInitialized = false;
	if (Key.Num() == 16)
	{
		AES128CTR.SetKey(Key.GetData());
		bIsInitialized = true;
		CryptoKey = Key;
		return IStreamDecrypterAES128::EResult::Ok;
	}
...
IStreamDecrypterAES128::EResult FStreamDecrypterAES128::CTRSetKey(const TArray<uint8>& Key)
{
	if (bIsInitialized)
	{
		if (Key.Num() == 16)
...
IStreamDecrypterAES128::EResult FStreamDecrypterAES128::CTRSetIV(const TArray<uint8>& IV)
{
	if (bIsInitialized)
	{
		if (IV.Num() == 8 || IV.Num() == 16)
		{
			AES128CTR.SetIV(IV);
			return IStreamDecrypterAES128::EResult::Ok;
		}
...
IStreamDecrypterAES128::EResult FStreamDecrypterAES128::CTRDecryptInPlace(uint8* InOutData, int32 InNumBytes)
{
	if (bIsInitialized)
	{
		AES128CTR.Decrypt(InOutData, InNumBytes);
		return IStreamDecrypterAES128::EResult::Ok;
	}
	return IStreamDecrypterAES128::EResult::NotInitialized;
}
