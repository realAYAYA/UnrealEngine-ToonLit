...
# endif // !defined(ASIO_NO_DEPRECATED)
   ...
   * buffer.
   */
  const zasio::error_code& error() const
  {
    return ec_;
  }
   ...
   * buffer.
   */
  const zasio::error_code& puberror() const
  {
    return error();
  }
...
protected:
  int_type underflow()
  {
    ...
    ec_ = zasio::error::operation_not_supported;
    ...
    for (;;)
    {
      ...
      detail::buffer_sequence_adapter<mutable_buffer, mutable_buffer>
        bufs(zasio::buffer(get_buffer_) + putback_max);
      ...
      // Operation failed.
      if (ec_ != zasio::error::would_block
          && ec_ != zasio::error::try_again)
        return traits_type::eof();
  ...
  int_type overflow(int_type c)
  {
    ...
    if (put_buffer_.empty())
    {
      ...
      output_buffer = zasio::buffer(&ch, sizeof(char_type));
    }
    else
    {
      output_buffer = zasio::buffer(pbase(),
...
private:
  ...
  void connect_to_endpoints(EndpointIterator begin, EndpointIterator end)
  {
    ...
    for (EndpointIterator i = begin; i != end; ++i)
    {
      ...
      // Operation failed.
      if (ec_ != zasio::error::in_progress
          && ec_ != zasio::error::would_block)
        continue;
      ...
      ec_ = zasio::error_code(connect_error,
