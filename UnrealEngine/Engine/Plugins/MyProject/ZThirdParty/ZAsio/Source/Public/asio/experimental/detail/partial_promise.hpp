...
struct partial_promise : partial_promise_base<Allocator>
{
  ...
  {
    return zasio::detail::suspend_always{};
  }
  ...
  auto final_suspend() noexcept
  {
    struct awaitable_t
    {
      ...
      auto await_suspend(zasio::detail::coroutine_handle<>) noexcept
      {
...
struct coroutine_traits<
    zasio::experimental::detail::partial_coro,
    Executor, Completion, Args...>
{
  using promise_type =
    zasio::experimental::detail::partial_promise<
      zasio::associated_allocator_t<Completion>>;
};
...
{
  post(exec, zasio::append(std::move(token), std::move(args)...));
  co_return;
}
...
partial_coro post_coroutine(Context& ctx,
    CompletionToken token, Args&&... args) noexcept
{
  post(ctx, zasio::append(std::move(token), std::move(args)...));
  co_return;
}
...
partial_coro dispatch_coroutine(Executor exec,
    CompletionToken token, Args&&... args) noexcept
{
  dispatch(exec, zasio::append(std::move(token), std::move(args)...));
  co_return;
}
...
partial_coro dispatch_coroutine(Context& ctx,
    CompletionToken token, Args &&... args) noexcept
{
  dispatch(ctx, zasio::append(std::move(token), std::move(args)...));
  co_return;
}
