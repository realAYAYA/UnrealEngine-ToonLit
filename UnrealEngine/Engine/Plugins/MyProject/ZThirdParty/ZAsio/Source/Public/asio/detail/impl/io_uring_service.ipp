...
io_uring_service::io_uring_service(zasio::execution_context& ctx)
...
void io_uring_service::notify_fork(
    zasio::execution_context::fork_event fork_ev)
{
  switch (fork_ev)
  {
  case zasio::execution_context::fork_prepare:
    {
  ...
  case zasio::execution_context::fork_parent:
    // Restart the timeout and eventfd operations.
    update_timeout();
    register_with_reactor();
    break;

  case zasio::execution_context::fork_child:
    {
    ...
    int op_type, io_uring_operation* op)
{
  ...
  {
    zasio::error_code ec(ENOBUFS,
        zasio::error::get_system_category());
    zasio::detail::throw_error(ec, "io_uring_get_sqe");
  }
}
...
void io_uring_service::register_buffers(const ::iovec* v, unsigned n)
{
  ...
  if (result < 0)
  {
    zasio::error_code ec(-result,
        zasio::error::get_system_category());
    zasio::detail::throw_error(ec, "io_uring_register_buffers");
  }
}
...
void io_uring_service::init_ring()
{
  ...
  if (result < 0)
  {
    ...
    zasio::error_code ec(-result,
        zasio::error::get_system_category());
    zasio::detail::throw_error(ec, "io_uring_queue_init");
  }
  ...
  if (event_fd_ < 0)
  {
    zasio::error_code ec(-result,
        zasio::error::get_system_category());
    ::io_uring_queue_exit(&ring_);
    zasio::detail::throw_error(ec, "eventfd");
  }
...
void io_uring_service::submit_sqes_op::do_complete(void* owner, operation* base,
    const zasio::error_code& /*ec*/, std::size_t /*bytes_transferred*/)
{
  if (owner)
  {
...
operation* io_uring_service::io_queue::perform_io(int result)
{
  ...
  if (result != -ECANCELED || cancel_requested_)
  {
    if (io_uring_operation* op = op_queue_.front())
    {
      if (result < 0)
      {
        op->ec_.assign(-result, zasio::error::get_system_category());
  ...
  if (!op_queue_.empty())
  {
    ...
    else
    {
      ...
      while (io_uring_operation* op = op_queue_.front())
      {
        op->ec_ = zasio::error::no_buffer_space;
        op_queue_.pop();
        io_cleanup.ops_.push(op);
      }
    }
