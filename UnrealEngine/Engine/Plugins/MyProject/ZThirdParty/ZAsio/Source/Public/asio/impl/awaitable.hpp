...
public:
  ...
  void* operator new(std::size_t size)
  {
    return zasio::detail::thread_info_base::allocate(
        zasio::detail::thread_info_base::awaitable_frame_tag(),
  ...
  void set_error(const zasio::error_code& ec)
  {
    this->set_except(std::make_exception_ptr(zasio::system_error(ec)));
  }
...
private:
  ...
  zasio::cancellation_slot parent_cancellation_slot_;
  zasio::cancellation_state cancellation_state_;
  bool has_executor_;
...
template <typename R, typename Executor>
class awaitable_async_op_handler<R(zasio::error_code), Executor>
  : public awaitable_thread<Executor>
{
public:
  typedef zasio::error_code* result_type;
...
template <typename R, typename T, typename Executor>
class awaitable_async_op_handler<R(zasio::error_code, T), Executor>
  : public awaitable_thread<Executor>
{
public:
  struct result_type
  ...
  void operator()(zasio::error_code ec, T value)
  {
    result_.ec_ = &ec;
...
template <typename R, typename... Ts, typename Executor>
class awaitable_async_op_handler<R(zasio::error_code, Ts...), Executor>
  : public awaitable_thread<Executor>
{
public:
  struct result_type
  ...
  template <typename... Args>
  void operator()(zasio::error_code ec, Args&&... args)
  {
    result_.ec_ = &ec;
...
struct coroutine_traits<zasio::awaitable<T, Executor>, Args...>
{
  typedef zasio::detail::awaitable_frame<T, Executor> promise_type;
};
