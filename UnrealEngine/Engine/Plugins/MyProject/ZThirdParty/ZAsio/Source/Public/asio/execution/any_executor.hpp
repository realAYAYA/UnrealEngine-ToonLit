...
public:
   ...
   * zasio::require and zasio::prefer customisation points.
   *
   * For example:
   * @code execution::any_executor<execution::blocking_t::possibly_t> ex = ...;
   * auto ex2 = zasio::requre(ex, execution::blocking.possibly); @endcode
   */
  ...
  /**
   * Do not call this function directly. It is intended for use with the
   * zasio::prefer customisation point.
   *
   * For example:
   * @code execution::any_executor<execution::blocking_t::possibly_t> ex = ...;
   * auto ex2 = zasio::prefer(ex, execution::blocking.possibly); @endcode
   */
  template <typename Property>
   ...
   * size_t n = zasio::query(ex, execution::occupancy); @endcode
   */
  template <typename Property>
  ...
  /**
   * Throws zasio::bad_executor if the polymorphic wrapper has no target.
   */
...
public:
  ...
  {
    if (impl_)
      zasio::detail::ref_count_up(impl_->ref_count_);
  }
  ...
  {
    if (impl_)
      if (zasio::detail::ref_count_down(impl_->ref_count_))
        delete impl_;
  }
...
private:
  struct impl_base
  {
    ...
    virtual ~impl_base() {}
    virtual void* get() = 0;
    zasio::detail::atomic_count ref_count_;
  };
...
public:
  ...
  void execute(ASIO_MOVE_ARG(F) f) const
  {
    if (target_)
    {
      ...
      {
        zasio::detail::non_const_lvalue<F> f2(f);
...
protected:
  ...
  typedef zasio::detail::executor_function function;
  typedef zasio::detail::executor_function_view function_view;
...
# pragma warning (disable:4702)
  ...
  static void query_fn_non_void(void*, const void* ex, const void* prop,
      typename enable_if<
        zasio::can_query<const Ex&, const Prop&>::value
          && is_same<typename Prop::polymorphic_query_result_type, void>::value
      >::type*)
  {
    zasio::query(*static_cast<const Ex*>(ex),
  ...
  static Poly require_fn_impl(const void* ex, const void* prop,
      ...
      >::type*)
  {
    return zasio::require(*static_cast<const Ex*>(ex),
        *static_cast<const Prop*>(prop));
  }
  ...
  static Poly prefer_fn_impl(const void* ex, const void* prop,
      ...
      >::type*)
  {
    return zasio::prefer(*static_cast<const Ex*>(ex),
        *static_cast<const Prop*>(prop));
  }
...
private:
  ...
  {
    return zasio::query(ex, execution::blocking);
  }
...
//  template <typename...> friend class any_executor;
  ...
  typedef aligned_storage<
      sizeof(zasio::detail::shared_ptr<void>) + sizeof(void*),
      alignment_of<zasio::detail::shared_ptr<void> >::value
    >::type object_type;
...
public:
      ...
      >::type* = 0) const
  {
    ...
    prop_fns_[found::index].query(&result, object_fns_->target(*this),
        &static_cast<const typename found::type&>(p));
    return *zasio::detail::scoped_ptr<
      typename found::query_result_type>(result);
  }
