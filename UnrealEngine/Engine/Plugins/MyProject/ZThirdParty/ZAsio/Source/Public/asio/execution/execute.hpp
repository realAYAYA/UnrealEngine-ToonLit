...
namespace zasio_execution_execute_fn {

using zasio::conditional;
using zasio::decay;
using zasio::declval;
using zasio::enable_if;
using zasio::execution::detail::as_receiver;
using zasio::execution::detail::is_as_invocable;
using zasio::execution::is_sender_to;
using zasio::false_type;
using zasio::result_of;
using zasio::traits::execute_free;
using zasio::traits::execute_member;
using zasio::true_type;
using zasio::void_type;
...
struct impl
{
  ...
  operator()(
    ...
    ASIO_NOEXCEPT_IF((
      call_traits<impl, T, void(F)>::is_noexcept))
  {
    return zasio::execution::detail::submit_helper(
        ASIO_MOVE_CAST(T)(t),
...
static ASIO_CONSTEXPR const zasio_execution_execute_fn::impl&
  execute = zasio_execution_execute_fn::static_instance<>::instance;

} // namespace

typedef zasio_execution_execute_fn::impl execute_t;

template <typename T, typename F>
struct can_execute :
  integral_constant<bool,
    zasio_execution_execute_fn::call_traits<
      execute_t, T, void(F)>::overload !=
        zasio_execution_execute_fn::ill_formed>
{
};
