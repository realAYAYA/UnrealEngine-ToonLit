  ...
  : ssl_(::SSL_new(context))
{
  if (!ssl_)
  {
    ...
    zasio::detail::throw_error(ec, "engine");
  }
...
zasio::error_code engine::set_verify_mode(
...
zasio::error_code engine::set_verify_depth(
...
zasio::error_code engine::set_verify_callback(
    verify_callback_base* callback, zasio::error_code& ec)
{
...
engine::want engine::handshake(
    stream_base::handshake_type type, zasio::error_code& ec)
{
  return perform((type == zasio::ssl::stream_base::client)
      ? &engine::do_connect : &engine::do_accept, 0, 0, ec, 0);
}

engine::want engine::shutdown(zasio::error_code& ec)
{
  return perform(&engine::do_shutdown, 0, 0, ec, 0);
}

engine::want engine::write(const zasio::const_buffer& data,
    zasio::error_code& ec, std::size_t& bytes_transferred)
{
  if (data.size() == 0)
  {
    ec = zasio::error_code();
...
engine::want engine::read(const zasio::mutable_buffer& data,
    zasio::error_code& ec, std::size_t& bytes_transferred)
{
  if (data.size() == 0)
  {
...
zasio::mutable_buffer engine::get_output(
    const zasio::mutable_buffer& data)
{
  int length = ::BIO_read(ext_bio_,
      data.data(), static_cast<int>(data.size()));

  return zasio::buffer(data,
      length > 0 ? static_cast<std::size_t>(length) : 0);
}

zasio::const_buffer engine::put_input(
...
const zasio::error_code& engine::map_error_code(
    zasio::error_code& ec) const
{
  ...
  {
    ec = zasio::ssl::error::stream_truncated;
    return ec;
  }
...
#if (OPENSSL_VERSION_NUMBER < 0x10000000L)
zasio::detail::static_mutex& engine::accept_mutex()
{
  static zasio::detail::static_mutex mutex = ASIO_STATIC_MUTEX_INIT;
  return mutex;
}
#endif // (OPENSSL_VERSION_NUMBER < 0x10000000L)

engine::want engine::perform(int (engine::* op)(void*, std::size_t),
    void* data, std::size_t length, zasio::error_code& ec,
    std::size_t* bytes_transferred)
{
  ...
  if (ssl_error == SSL_ERROR_SYSCALL)
  {
    if (sys_error == 0)
    {
      ec = zasio::ssl::error::unspecified_system_error;
    }
    else
    {
      ec = zasio::error_code(sys_error,
  ...
  {
    ec = zasio::ssl::error::unexpected_result;
    return want_nothing;
  }
}
...
{
#if (OPENSSL_VERSION_NUMBER < 0x10000000L)
  zasio::detail::static_mutex::scoped_lock lock(accept_mutex());
#endif // (OPENSSL_VERSION_NUMBER < 0x10000000L)
