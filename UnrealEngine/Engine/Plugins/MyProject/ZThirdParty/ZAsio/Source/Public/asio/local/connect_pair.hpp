...
ASIO_SYNC_OP_VOID connect_pair(basic_socket<Protocol, Executor1>& socket1,
    basic_socket<Protocol, Executor2>& socket2, zasio::error_code& ec);
    ...
    basic_socket<Protocol, Executor2>& socket2)
{
  ...
  zasio::detail::throw_error(ec, "connect_pair");
}
    ...
    basic_socket<Protocol, Executor2>& socket2, zasio::error_code& ec)
{
  ...
  zasio::local::basic_endpoint<Protocol>* tmp
    = static_cast<typename Protocol::endpoint*>(0);
  (void)tmp;

  Protocol protocol;
  zasio::detail::socket_type sv[2];
  if (zasio::detail::socket_ops::socketpair(protocol.family(),
        protocol.type(), protocol.protocol(), sv, ec)
      == zasio::detail::socket_error_retval)
    ASIO_SYNC_OP_VOID_RETURN(ec);

  socket1.assign(protocol, sv[0], ec);
  if (ec)
  {
    zasio::error_code temp_ec;
    zasio::detail::socket_ops::state_type state[2] = { 0, 0 };
    zasio::detail::socket_ops::close(sv[0], state[0], true, temp_ec);
    zasio::detail::socket_ops::close(sv[1], state[1], true, temp_ec);
    ASIO_SYNC_OP_VOID_RETURN(ec);
  }
  ...
  if (ec)
  {
    ...
    zasio::detail::socket_ops::state_type state = 0;
    zasio::detail::socket_ops::close(sv[1], state, true, temp_ec);
    ASIO_SYNC_OP_VOID_RETURN(ec);
  }
