...
void zasio_signal_handler(int signal_number)
{
  ...
  ::signal(signal_number, zasio_signal_handler);
...
# if defined(ASIO_HAS_IO_URING_AS_DEFAULT)
  pipe_read_op()
    : io_uring_operation(zasio::error_code(), &pipe_read_op::do_prepare,
...
signal_set_service::signal_set_service(execution_context& context)
  : execution_context_service_base<signal_set_service>(context),
    scheduler_(zasio::use_service<scheduler_impl>(context)),
#if !defined(ASIO_WINDOWS) \
  && !defined(ASIO_WINDOWS_RUNTIME) \
  && !defined(__CYGWIN__)
# if defined(ASIO_HAS_IO_URING_AS_DEFAULT)
    io_uring_service_(zasio::use_service<io_uring_service>(context)),
# else // defined(ASIO_HAS_IO_URING_AS_DEFAULT)
    reactor_(zasio::use_service<reactor>(context)),
# endif // defined(ASIO_HAS_IO_URING_AS_DEFAULT)
#endif // !defined(ASIO_WINDOWS)
       //   && !defined(ASIO_WINDOWS_RUNTIME)
       //   && !defined(__CYGWIN__)
...
# endif // defined(ASIO_HAS_IO_URING_AS_DEFAULT)
  ...
  case execution_context::fork_child:
    if (state->fork_prepared_)
    {
      zasio::detail::signal_blocker blocker;
      close_descriptors();
      open_descriptors();
      int read_descriptor = state->read_descriptor_;
      state->fork_prepared_ = false;
...
zasio::error_code signal_set_service::add(
    signal_set_service::implementation_type& impl, int signal_number,
    signal_set_base::flags_t f, zasio::error_code& ec)
{
...
# if defined(ASIO_HAS_SIGACTION)
      ...
      sa.sa_handler = zasio_signal_handler;
      sigfillset(&sa.sa_mask);
      if (f != signal_set_base::flags::dont_care)
        sa.sa_flags = static_cast<int>(f);
      if (::sigaction(signal_number, &sa, 0) == -1)
# else // defined(ASIO_HAS_SIGACTION)
      if (::signal(signal_number, zasio_signal_handler) == SIG_ERR)
# endif // defined(ASIO_HAS_SIGACTION)
      {
...
zasio::error_code signal_set_service::remove(
    signal_set_service::implementation_type& impl,
    int signal_number, zasio::error_code& ec)
{
  // Check that the signal number is valid.
  if (signal_number < 0 || signal_number >= max_signal_number)
  {
...
zasio::error_code signal_set_service::clear(
    signal_set_service::implementation_type& impl,
    zasio::error_code& ec)
{
  signal_state* state = get_signal_state();
...
zasio::error_code signal_set_service::cancel(
    signal_set_service::implementation_type& impl,
    zasio::error_code& ec)
{
  ...
  op_queue<operation> ops;
  {
    ...
    {
      op->ec_ = zasio::error::operation_aborted;
      impl.queue_.pop();
      ops.push(op);
    }
  }
...
void signal_set_service::open_descriptors()
{
  ...
  {
    zasio::error_code ec(errno,
        zasio::error::get_system_category());
    zasio::detail::throw_error(ec, "signal_set_service pipe");
  }
#endif // !defined(ASIO_WINDOWS)
       //   && !defined(ASIO_WINDOWS_RUNTIME)
       //   && !defined(__CYGWIN__)
