...
class channel_service
  : public zasio::detail::execution_context_service_base<
...
public:
  ...
  struct base_implementation_type
  {
    ...
    zasio::detail::op_queue<channel_operation> waiters_;
...
struct channel_service<Mutex>::implementation_type<
    Traits, R(zasio::error_code)>
  : channel_service::base_implementation_type
{
  // The traits type associated with the channel.
  typedef typename Traits::template rebind<R(zasio::error_code)>::other
    traits_type;
  ...
  typedef typename conditional<
      ...
      typename conditional<
        has_signature<
          ...
          R(zasio::error_code)
        >::value,
        channel_payload<R(zasio::error_code)>,
        channel_payload<
          R(zasio::error_code),
          typename traits_type::receive_closed_signature
        >
  ...
  void buffer_push(payload_type payload)
  {
    ...
    {
      zasio::error_code value{last.value_};
...
private:
  ...
  {
    zasio::error_code value_;
    std::size_t count_;
  };

  struct value_handler
  {
    zasio::error_code& target_;

    template <typename... Args>
    void operator()(const zasio::error_code& value, Args&&...)
    {
      target_ = value;
    }
  };
