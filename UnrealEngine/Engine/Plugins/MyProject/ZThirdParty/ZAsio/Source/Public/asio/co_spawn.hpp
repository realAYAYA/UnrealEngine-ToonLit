 ...
 * execution.
 *
 * @param a The zasio::awaitable object that is the result of calling the
 ...
 * @par Example
 * @code
 * zasio::awaitable<std::size_t> echo(tcp::socket socket)
 * {
 ...
 *           zasio::buffer(data), zasio::use_awaitable);
 *
 *       co_await zasio::async_write(socket,
 *           zasio::buffer(data, n), zasio::use_awaitable);
 *
 *       bytes_transferred += n;
 *     }
 *   }
 ...
 * cancellation state, call zasio::this_coro::reset_cancellation_state.
 */
 ...
 * @par Example
 * @code
 * zasio::awaitable<void> echo(tcp::socket socket)
 * {
 *   try
 *   {
 *     char data[1024];
 ...
 * @c zasio::awaitable<R,E> that will be used as the coroutine's entry
 * point.
 *
 ...
 * @code zasio::awaitable<R, AwaitableExecutor> F() @endcode
 *
 * @par Example
 * @code
 * zasio::awaitable<std::size_t> echo(tcp::socket socket)
