...
public:
  ...
  std::size_t fill(zasio::error_code& ec);
  ...
  /// Determine the amount of data that may be read without blocking.
  std::size_t in_avail(zasio::error_code& ec)
  {
    ec = zasio::error_code();
    return storage_.size();
  }
...
private:
  ...
  {
    std::size_t bytes_copied = zasio::buffer_copy(
  ...
  {
    return zasio::buffer_copy(buffers, storage_.data(), storage_.size());
  }
