...
template<typename Signature = void(),
    typename Executor = zasio::any_io_executor,
...
struct promise_impl<void(Ts...), Executor, Allocator>
{
  ...
  void apply_impl(Func f, zasio::detail::index_sequence<Idx...>)
  {
  ...
  void apply(Func f)
  {
    apply_impl(std::forward<Func>(f),
        zasio::detail::make_index_sequence<sizeof...(Ts)>{});
  }
  ...
  struct completion_impl final : completion_base
  {
    ...
    void invoke(Ts&&... ts)
    {
      ...
      using alloc_t = typename std::allocator_traits<
        typename zasio::decay<Alloc>::type>::template
  ...
  typename zasio::aligned_storage<sizeof(void*) * 4,
  ...
  void set_completion(Alloc&& alloc, Handler&& handler)
  {
    ...
    using impl_t = completion_impl<
      typename zasio::decay<Alloc>::type, Handler>;
    using alloc_t = typename std::allocator_traits<
      typename zasio::decay<Alloc>::type>::template rebind_alloc<impl_t>;
  ...
  void complete_with_result_impl(zasio::detail::index_sequence<Idx...>)
  {
    auto& result_type = *reinterpret_cast<promise_impl::result_type*>(&result);
  ...
  template<typename... T_>
  void cancel_impl_(zasio::error_code*, T_*...)
  {
    complete(zasio::error::operation_aborted, T_{}...);
  }
