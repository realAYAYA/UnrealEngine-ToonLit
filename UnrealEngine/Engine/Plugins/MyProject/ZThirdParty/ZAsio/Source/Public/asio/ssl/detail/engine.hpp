...
public:
  ...
  ASIO_DECL zasio::error_code set_verify_mode(
      verify_mode v, zasio::error_code& ec);

  // Set the peer verification depth.
  ASIO_DECL zasio::error_code set_verify_depth(
      int depth, zasio::error_code& ec);

  // Set a peer certificate verification callback.
  ASIO_DECL zasio::error_code set_verify_callback(
  ...
  ASIO_DECL want shutdown(zasio::error_code& ec);

  // Write bytes to the SSL session.
  ASIO_DECL want write(const zasio::const_buffer& data,
      zasio::error_code& ec, std::size_t& bytes_transferred);

  // Read bytes from the SSL session.
  ASIO_DECL want read(const zasio::mutable_buffer& data,
      zasio::error_code& ec, std::size_t& bytes_transferred);

  // Get output data to be written to the transport.
  ASIO_DECL zasio::mutable_buffer get_output(
      const zasio::mutable_buffer& data);

  // Put input data that was read from the transport.
  ASIO_DECL zasio::const_buffer put_input(
      const zasio::const_buffer& data);

  // Map an error::eof code returned by the underlying transport according to
  // the type and state of the SSL session. Returns a const reference to the
  // error code object, suitable for passing to a completion handler.
  ASIO_DECL const zasio::error_code& map_error_code(
...
private:
  ...
  ASIO_DECL static zasio::detail::static_mutex& accept_mutex();
