...
public:
  ...
  {
    return zasio::query(system_executor(), execution::context);
  }
  ...
  void execute(ASIO_MOVE_ARG(F) f) const
  {
    ...
    zasio::require(system_executor(), Blocking()).execute(
        promise_invoker<T, F>(p_, ASIO_MOVE_CAST(F)(f)));
#else // defined(ASIO_NO_DEPRECATED)
    execution::execute(
        zasio::require(system_executor(), Blocking()),
        promise_invoker<T, F>(p_, ASIO_MOVE_CAST(F)(f)));
...
template <>
class promise_handler_selector<void(zasio::error_code)>
...
template <typename Arg>
class promise_handler_selector<void(zasio::error_code, Arg)>
...
#define ASIO_PRIVATE_PROMISE_SELECTOR_DEF(n) \
  ...
  template <typename Arg, ASIO_VARIADIC_TPARAMS(n)> \
  class promise_handler_selector< \
    void(zasio::error_code, Arg, ASIO_VARIADIC_TARGS(n))> \
    ...
    promise_handler<Signature, Allocator>* h)
{
  typename promise_handler<Signature, Allocator>::executor_type
    ex(h->get_executor());
  zasio::dispatch(ex, promise_function_wrapper<Function>(f));
}
...
struct equality_comparable<
    zasio::detail::promise_executor<T, Blocking> >
{
...
struct execute_member<
    zasio::detail::promise_executor<T, Blocking>, Function>
{
  ASIO_STATIC_CONSTEXPR(bool, is_valid = true);
  ...
  >
{
  ASIO_STATIC_CONSTEXPR(bool, is_valid = true);
  ASIO_STATIC_CONSTEXPR(bool, is_noexcept = true);
  typedef zasio::detail::promise_executor<T,
