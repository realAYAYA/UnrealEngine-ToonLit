...
/**
 * The zasio::ip::address_v6 class provides the ability to use and
...
#else
  typedef zasio::detail::array<unsigned char, 16> bytes_type;
#endif
...
private:
  ...
  zasio::detail::in6_addr_type addr_;
...
template <>
struct hash<zasio::ip::address_v6>
{
  std::size_t operator()(const zasio::ip::address_v6& addr)
    const ASIO_NOEXCEPT
  {
    const zasio::ip::address_v6::bytes_type bytes = addr.to_bytes();
