...
void winrt_ssocket_service_base::base_shutdown()
{
  ...
  zasio::detail::mutex::scoped_lock lock(mutex_);
    ...
    winrt_ssocket_service_base::base_implementation_type& other_impl)
{
  ...
  {
    // Insert implementation into linked list of all implementations.
    zasio::detail::mutex::scoped_lock lock(other_service.mutex_);
...
zasio::error_code winrt_ssocket_service_base::close(
    winrt_ssocket_service_base::base_implementation_type& impl,
    zasio::error_code& ec)
{
  delete impl.socket_;
...
zasio::error_code winrt_ssocket_service_base::do_set_option(
    winrt_ssocket_service_base::base_implementation_type& impl,
...
zasio::error_code winrt_ssocket_service_base::do_connect(
    winrt_ssocket_service_base::base_implementation_type& impl,
    ...
    const void* addr, winrt_async_op<void>* op, bool is_continuation)
{
  if (!is_open(impl))
  {
    op->ec_ = zasio::error::bad_descriptor;
  ...
  case ASIO_OS_DEF(AF_INET6):
    ...
    break;
  default:
    op->ec_ = zasio::error::address_family_not_supported;
    break;
  }
  ...
  {
    op->ec_ = zasio::error_code(
        e->HResult, zasio::system_category());
    scheduler_.post_immediate_completion(op, is_continuation);
  }
}
    ...
    socket_base::message_flags flags, zasio::error_code& ec)
{
  ...
  try
  {
    buffer_sequence_adapter<zasio::const_buffer,
      zasio::const_buffer> bufs(zasio::buffer(data));
      ...
      winrt_async_op<unsigned int>* op, bool is_continuation)
{
  if (flags)
  {
    op->ec_ = zasio::error::operation_not_supported;
    scheduler_.post_immediate_completion(op, is_continuation);
    return;
  }
  ...
  {
    op->ec_ = zasio::error_code(e->HResult,
        zasio::system_category());
    scheduler_.post_immediate_completion(op, is_continuation);
  }
}
...
std::size_t winrt_ssocket_service_base::do_receive(
    winrt_ssocket_service_base::base_implementation_type& impl,
    const zasio::mutable_buffer& data,
    socket_base::message_flags flags, zasio::error_code& ec)
{
  if (flags)
  {
  ...
  try
  {
    buffer_sequence_adapter<zasio::mutable_buffer,
        zasio::mutable_buffer> bufs(zasio::buffer(data));
