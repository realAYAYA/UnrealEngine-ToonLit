...
struct prefer_only_property<InnerProperty,
    typename void_type<
      decltype(zasio::declval<const InnerProperty>().value())
    >::type>
{
  InnerProperty property;
  ...
  ASIO_CONSTEXPR auto value() const
    ASIO_NOEXCEPT_IF((
      noexcept(zasio::declval<const InnerProperty>().value())))
    -> decltype(zasio::declval<const InnerProperty>().value())
  {
  ...
  && !defined(__clang__) // Clang crashes if noexcept is used here.
    ASIO_NOEXCEPT_IF((
      is_nothrow_prefer<const Executor&, const InnerProperty&>::value))
       ...
       //   && !defined(__clang__)
  {
    return zasio::prefer(ex, p.property);
  }
  ...
  && !defined(__clang__) // Clang crashes if noexcept is used here.
    ASIO_NOEXCEPT_IF((
      is_nothrow_query<const Executor&, const InnerProperty&>::value))
       ...
       //   && !defined(__clang__)
  {
    return zasio::query(ex, p.property);
  }
};
