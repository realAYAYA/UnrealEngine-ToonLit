...
public:
  ...
  ASIO_DECL win_iocp_io_context(zasio::execution_context& ctx,
  ...
  ASIO_DECL zasio::error_code register_handle(
      HANDLE handle, zasio::error_code& ec);

  // Run the event loop until stopped or no more work.
  ASIO_DECL size_t run(zasio::error_code& ec);

  // Run until stopped or one operation is performed.
  ASIO_DECL size_t run_one(zasio::error_code& ec);

  // Run until timeout, interrupted, or one operation is performed.
  ASIO_DECL size_t wait_one(long usec, zasio::error_code& ec);

  // Poll for operations without blocking.
  ASIO_DECL size_t poll(zasio::error_code& ec);

  // Poll for one operation without blocking.
  ASIO_DECL size_t poll_one(zasio::error_code& ec);
  ...
  // starting the operation.
  ASIO_DECL void on_completion(win_iocp_operation* op,
      const zasio::error_code& ec, DWORD bytes_transferred = 0);
