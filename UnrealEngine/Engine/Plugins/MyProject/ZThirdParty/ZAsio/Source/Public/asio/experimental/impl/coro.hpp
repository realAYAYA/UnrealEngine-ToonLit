...
template <>
struct coro_error<zasio::error_code>
{
  static zasio::error_code invalid()
  {
    return zasio::error::fault;
  }

  static zasio::error_code cancelled()
  {
    return zasio::error::operation_aborted;
  }

  static zasio::error_code interrupted()
  {
    return zasio::error::interrupted;
  }

  static zasio::error_code done()
  {
    return zasio::error::broken_pipe;
  }
};
...
struct coro_error<std::exception_ptr>
{
  static std::exception_ptr invalid()
  {
    return std::make_exception_ptr(
        zasio::system_error(
          coro_error<zasio::error_code>::invalid()));
  }
  ...
  static std::exception_ptr cancelled()
  {
    return std::make_exception_ptr(
        zasio::system_error(
          coro_error<zasio::error_code>::cancelled()));
  }
  ...
  static std::exception_ptr interrupted()
  {
    return std::make_exception_ptr(
        zasio::system_error(
          coro_error<zasio::error_code>::interrupted()));
  }
  ...
  static std::exception_ptr done()
  {
    return std::make_exception_ptr(
        zasio::system_error(
          coro_error<zasio::error_code>::done()));
  }
};
      ...
      -> coroutine_handle<>
    {
      ...
      if constexpr (!coro_promise<Y, R, E, A>::is_noexcept)
      {
            ...
            && hp.cancel->throw_if_cancelled_)
        {
          zasio::detail::throw_error(
              zasio::error::operation_aborted, "coro-cancelled");
        }
      }
      ...
      else
      {
        ...
        struct cancel_handler
        {
          ...
          {
            zasio::dispatch(e, [ct, st = st]() mutable
            {
              auto & [sig, state] = *st;
              sig.emit(ct);
            });
      ...
      typename coro_traits<Yield, Return, Executor>::return_type>
{
  ...
  using cancellation_slot_type = zasio::cancellation_slot;
  ...
  auto await_transform(this_coro::cancellation_state_t) const
  {
    struct exec_helper
    {
      const zasio::cancellation_state& value;
      ...
      zasio::cancellation_state await_resume() const noexcept
      {
        return value;
      }
    };
...
struct coro<Yield, Return, Executor, Allocator>::initiate_async_resume
{
  ...
  typedef zasio::cancellation_slot cancellation_slot_type;
      ...
      std::true_type /* result is void */)
  {
    ...
    {
      if (!coro)
        return zasio::post(exec,
            zasio::append(std::move(h),
