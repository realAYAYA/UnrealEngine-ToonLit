...
address_v4::address_v4(address_v4::uint_type addr)
{
  ...
  addr_.s_addr = zasio::detail::socket_ops::host_to_network_long(
      static_cast<zasio::detail::u_long_type>(addr));
}
...
{
  return zasio::detail::socket_ops::network_to_host_long(addr_.s_addr);
}
...
{
  zasio::error_code ec;
  char addr_str[zasio::detail::max_addr_v4_str_len];
  const char* addr =
    zasio::detail::socket_ops::inet_ntop(
        ASIO_OS_DEF(AF_INET), &addr_, addr_str,
        zasio::detail::max_addr_v4_str_len, 0, ec);
  if (addr == 0)
...
#if !defined(ASIO_NO_DEPRECATED)
std::string address_v4::to_string(zasio::error_code& ec) const
{
  char addr_str[zasio::detail::max_addr_v4_str_len];
  const char* addr =
    zasio::detail::socket_ops::inet_ntop(
    ...
    zasio::error_code& ec) ASIO_NOEXCEPT
{
  ...
  if (zasio::detail::socket_ops::inet_pton(
