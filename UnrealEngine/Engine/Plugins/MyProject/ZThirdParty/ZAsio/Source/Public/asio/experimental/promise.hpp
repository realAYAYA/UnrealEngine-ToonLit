 ...
 * @code
 * awaitable<void> read_write_some(zasio::ip::tcp::socket & sock,
 *     zasio::mutable_buffer read_buf, zasio::const_buffer to_write)
 * {
 *   auto p = zasio::async_read(read_buf, zasio::use_awaitable);
 *   co_await zasio::async_write_some(to_write, zasio::deferred);
 *   co_await p;
 * }
 * @endcode
 */
...
private:
  ...
  struct initiate_async_wait
  {
    ...
    void operator()(WaitHandler&& handler) const
    {
      ...
      if (self_->done)
      {
        auto exec = zasio::get_associated_executor(
