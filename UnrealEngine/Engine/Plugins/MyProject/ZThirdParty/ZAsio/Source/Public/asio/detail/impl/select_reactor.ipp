...
select_reactor::select_reactor(zasio::execution_context& ctx)
...
{
#if defined(ASIO_HAS_IOCP)
  zasio::detail::signal_blocker sb;
  thread_ = new zasio::detail::thread(thread_function(this));
#endif // defined(ASIO_HAS_IOCP)
}
    ...
    zasio::execution_context::fork_event fork_ev)
{
  ...
  (void)fork_ev;
#else // defined(ASIO_HAS_IOCP)
  if (fork_ev == zasio::execution_context::fork_child)
...
{
  zasio::detail::mutex::scoped_lock lock(mutex_);
  cancel_ops_unlocked(descriptor, zasio::error::operation_aborted);
}
    ...
    int op_type, void* cancellation_key)
{
  ...
  bool need_interrupt = op_queue_[op_type].cancel_operations_by_key(
      descriptor, ops, cancellation_key, zasio::error::operation_aborted);
    ...
    const zasio::error_code& /*ec*/, std::size_t /*bytes_transferred*/)
{
  if (owner)
  {
    ...
    zasio::detail::mutex::scoped_lock lock(reactor->mutex_);
    ...
    zasio::detail::signal_blocker sb;
    reactor->thread_ =
      new zasio::detail::thread(thread_function(reactor));
  }
}
#endif // defined(ASIO_HAS_IOCP)
