...
template <typename R>
struct channel_traits<R(zasio::error_code)>
{
  template <typename... NewSignatures>
  struct rebind
  {
  ...
  typedef R receive_cancelled_signature(zasio::error_code);

  template <typename F>
  static void invoke_receive_cancelled(F f)
  {
    const zasio::error_code e = error::channel_cancelled;
    ASIO_MOVE_OR_LVALUE(F)(f)(e);
  }

  typedef R receive_closed_signature(zasio::error_code);

  template <typename F>
  static void invoke_receive_closed(F f)
  {
    const zasio::error_code e = error::channel_closed;
    ASIO_MOVE_OR_LVALUE(F)(f)(e);
  }
};

template <typename R, typename... Args, typename... Signatures>
struct channel_traits<R(zasio::error_code, Args...), Signatures...>
{
  template <typename... NewSignatures>
  struct rebind
  {
  ...
  typedef R receive_cancelled_signature(zasio::error_code, Args...);
  ...
  typedef R receive_closed_signature(zasio::error_code, Args...);
...
struct channel_traits<R(std::exception_ptr)>
{
  ...
  static void invoke_receive_cancelled(F f)
  {
    ...
    ASIO_MOVE_OR_LVALUE(F)(f)(
        std::make_exception_ptr(zasio::system_error(e)));
  }
