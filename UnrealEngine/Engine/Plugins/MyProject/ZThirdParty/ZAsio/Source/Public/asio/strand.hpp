...
public:
   ...
   *
   * For example:
   * @code zasio::strand<my_executor_type> ex = ...;
   * if (zasio::query(ex, zasio::execution::blocking)
   *       == zasio::execution::blocking.never)
   *   ... @endcode
   */
   ...
   *
   * For example:
   * @code zasio::strand<my_executor_type> ex1 = ...;
      ...
      is_nothrow_require<const Executor&, Property>::value))
  {
    return strand<typename decay<
      typename require_result<const Executor&, Property>::type
        >::type>(zasio::require(executor_, p), impl_);
  }
      ...
      is_nothrow_prefer<const Executor&, Property>::value))
  {
    return strand<typename decay<
      typename prefer_result<const Executor&, Property>::type
        >::type>(zasio::prefer(executor_, p), impl_);
  }
...
private:
      ...
      >::type = 0)
  {
    return use_service<detail::strand_executor_service>(
        zasio::query(ex, execution::context)).create_implementation();
  }
  ...
  {
    return zasio::query(executor_, property);
  }

  template <typename Property>
  execution::blocking_t query_helper(true_type, const Property& property) const
  {
    execution::blocking_t result = zasio::query(executor_, property);
...
/** @defgroup make_strand zasio::make_strand
 *
 * @brief The zasio::make_strand function creates a @ref strand object for
