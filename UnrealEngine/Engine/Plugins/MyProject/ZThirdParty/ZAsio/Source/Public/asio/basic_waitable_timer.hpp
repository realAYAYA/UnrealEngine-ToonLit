...
template <typename Clock,
    typename WaitTraits = zasio::wait_traits<Clock>,
 ...
 *
 * Most applications will use one of the zasio::steady_timer,
 * zasio::system_timer or zasio::high_resolution_timer typedefs.
 *
 ...
 * zasio::steady_timer timer(my_context);
 *
 ...
 * @code
 * void handler(const zasio::error_code& error)
 * {
 *   if (!error)
 *   {
 *     // Timer expired.
 ...
 * }
 *
 * void on_timeout(const zasio::error_code& e)
 * {
 *   if (e != zasio::error::operation_aborted)
 *   {
 ...
 * @endcode
 *
 * @li The zasio::basic_waitable_timer::expires_after() function
 * cancels any pending asynchronous waits, and returns the number of
 * asynchronous waits that were cancelled. If it returns 0 then you were too
 * late and the wait handler has already been executed, or will soon be
 * executed. If it returns 1 then the wait handler was successfully cancelled.
 *
 * @li If a wait handler is cancelled, the zasio::error_code passed to
 * it contains the value zasio::error::operation_aborted.
 */
...
public:
    ...
    : impl_(0, ex)
  {
    ...
    zasio::detail::throw_error(ec, "expires_at");
  }
    ...
    : impl_(0, ex)
  {
    ...
    zasio::detail::throw_error(ec, "expires_after");
  }
   ...
   * be invoked with the zasio::error::operation_aborted error code.
   *
   ...
   * error code that indicates the successful completion of the wait operation.
   */
  std::size_t cancel(zasio::error_code& ec)
  {
   ...
   * zasio::error::operation_aborted error code.
   *
   * Cancelling the timer does not change the expiry time.
   *
  ...
  std::size_t cancel_one()
  {
    ...
    zasio::detail::throw_error(ec, "cancel_one");
    return s;
  }
   ...
   * error code that indicates the successful completion of the wait operation.
   */
  std::size_t cancel_one(zasio::error_code& ec)
  {
  ...
  std::size_t expires_from_now(const duration& expiry_time)
  {
    ...
    std::size_t s = impl_.get_service().expires_from_now(
        impl_.get_implementation(), expiry_time, ec);
    zasio::detail::throw_error(ec, "expires_from_now");
    return s;
  }
   ...
   * code zasio::error::operation_aborted.
   *
  ...
  ASIO_INITFN_AUTO_RESULT_TYPE_PREFIX(
      WaitToken, void (zasio::error_code))
  async_wait(
