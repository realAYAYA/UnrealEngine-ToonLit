...
struct parallel_group_completion_handler
{
  ...
  parallel_group_completion_handler(Handler&& h)
    : handler_(std::move(h)),
      executor_(
          zasio::prefer(
            zasio::get_associated_executor(handler_),
  ...
  void operator()()
  {
    this->invoke(zasio::detail::make_index_sequence<sizeof...(Ops)>());
  }

  template <std::size_t... I>
  void invoke(zasio::detail::index_sequence<I...>)
  {
  ...
  void invoke(std::tuple<Args...>&& args)
  {
    this->invoke(std::move(args),
        zasio::detail::index_sequence_for<Args...>());
  }

  template <typename... Args, std::size_t... I>
  void invoke(std::tuple<Args...>&& args,
      zasio::detail::index_sequence<I...>)
  {
...
struct parallel_group_state
{
  ...
  zasio::cancellation_signal cancellation_signals_[sizeof...(Ops)];
...
struct parallel_group_op_handler
{
  ...
  void operator()(Args... args)
  {
    ...
    if (--state_->outstanding_ == 0)
      zasio::dispatch(std::move(state_->handler_));
  }
  ...
  parallel_group_op_handler<I, Condition, Handler, Ops...>
{
  ...
  struct cancel_proxy
  {
    ...
    void operator()(cancellation_type_t type)
    {
      ...
      {
        zasio::cancellation_signal* sig = &signal_;
        zasio::dispatch(executor_,
            [state, sig, type]{ sig->emit(type); });
      }
    }

    std::weak_ptr<parallel_group_state<Condition, Handler, Ops...> > state_;
    zasio::cancellation_signal signal_;
    executor_type executor_;
  };
...
struct parallel_group_op_launcher
{
      ...
      Condition, Handler, Ops...> >& state)
  {
    ...
    ex_type ex = zasio::get_associated_executor(op);
    std::move(op)(
...
void parallel_group_launch(Condition cancellation_condition, Handler handler,
    std::tuple<Ops...>& ops, zasio::detail::index_sequence<I...>)
{
  ...
  std::shared_ptr<state_type> state = std::allocate_shared<state_type>(
      zasio::detail::recycling_allocator<state_type,
        zasio::detail::thread_info_base::parallel_group_tag>(),
...
struct ranged_parallel_group_completion_handler
{
  ...
  {
    this->invoke(
        zasio::detail::make_index_sequence<
...
struct ranged_parallel_group_state
{
    ...
    : cancellations_requested_(size),
      ...
      cancellation_signals_(
          ASIO_REBIND_ALLOC(Allocator,
            zasio::cancellation_signal)(allocator)),
      cancellation_condition_(std::move(c)),
  ...
  // The cancellation signals for each operation in the group.
  std::deque<zasio::cancellation_signal,
    ASIO_REBIND_ALLOC(Allocator, zasio::cancellation_signal)>
      cancellation_signals_;
