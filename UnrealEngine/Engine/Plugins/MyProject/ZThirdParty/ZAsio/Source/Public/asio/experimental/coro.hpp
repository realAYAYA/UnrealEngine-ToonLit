...
struct coro
{
  /// The traits of the coroutine. See zasio::experimental::coro_traits
  /// for details.
  ...
  ~coro()
  {
    if (coro_ != nullptr)
    {
      ...
      if (handle)
        zasio::dispatch(coro_->get_executor(), destroyer{handle});
    }
  }
...
template<typename T, typename Executor = zasio::any_io_executor,
    typename Allocator = std::allocator<void>>
