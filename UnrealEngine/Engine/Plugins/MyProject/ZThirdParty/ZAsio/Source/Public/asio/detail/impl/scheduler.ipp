...
scheduler::scheduler(zasio::execution_context& ctx,
    int concurrency_hint, bool own_thread, get_task_func_type get_task)
  : zasio::detail::execution_context_service_base<scheduler>(ctx),
...
std::size_t scheduler::run(zasio::error_code& ec)
{
  ec = zasio::error_code();
...
std::size_t scheduler::run_one(zasio::error_code& ec)
{
  ec = zasio::error_code();
  if (outstanding_work_ == 0)
  {
...
std::size_t scheduler::wait_one(long usec, zasio::error_code& ec)
{
  ec = zasio::error_code();
  if (outstanding_work_ == 0)
  {
...
std::size_t scheduler::poll(zasio::error_code& ec)
{
  ec = zasio::error_code();
  if (outstanding_work_ == 0)
  {
...
std::size_t scheduler::poll_one(zasio::error_code& ec)
{
  ec = zasio::error_code();
  if (outstanding_work_ == 0)
  {
...
scheduler_task* scheduler::get_default_task(zasio::execution_context& ctx)
{
#if defined(ASIO_HAS_IO_URING_AS_DEFAULT)
