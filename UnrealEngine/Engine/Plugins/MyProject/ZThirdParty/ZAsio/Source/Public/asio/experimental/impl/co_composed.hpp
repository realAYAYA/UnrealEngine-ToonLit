...
using zasio::detail::composed_io_executors;
using zasio::detail::composed_work;
using zasio::detail::composed_work_guard;
using zasio::detail::get_composed_io_executor;
using zasio::detail::make_composed_io_executors;
using zasio::detail::recycling_allocator;
using zasio::detail::throw_error;
...
struct co_composed_state_default_cancellation_on_suspend_impl<Executors,
    Handler, Return, R(zasio::error_code, Args...), Signatures...>
{
  using promise_type = co_composed_promise<Executors, Handler, Return>;
  using return_type = std::tuple<zasio::error_code, Args...>;
  ...
  static constexpr void (*fn())(void*)
  {
    if constexpr ((is_constructible<Args>::value && ...))
    {
      return [](void* p)
      {
        ...
        std::move(handler)(
            zasio::error_code(zasio::error::operation_aborted),
            Args{}...);
      };
    }
    else
      ...
      return [](void* p)
      {
        ...
        std::move(handler)(
            std::make_exception_ptr(
              zasio::system_error(
                zasio::error::operation_aborted, "co_await")),
            Args{}...);
      };
    }
    else
...
private:
  ...
  void check_for_cancellation_on_transform()
  {
    if (throw_if_cancelled_ && !!cancelled())
      throw_error(zasio::error::operation_aborted, "co_await");
  }
...
class co_composed_handler<Executors, Handler,
    Return, R(zasio::error_code, Args...)>
  : public co_composed_handler_base<Executors, Handler, Return>
{
public:
  using co_composed_handler_base<Executors,
  ...
  using result_type = std::tuple<zasio::error_code, args_type>;

  template <typename... T>
  void operator()(const zasio::error_code& ec, T&&... args)
  {
...
#if defined(ASIO_ENABLE_HANDLER_TRACKING)
# if defined(ASIO_HAS_SOURCE_LOCATION)
      , zasio::detail::source_location location
        = zasio::detail::source_location::current()
# endif // defined(ASIO_HAS_SOURCE_LOCATION)
#endif // defined(ASIO_ENABLE_HANDLER_TRACKING)
    )
  {
...
#if defined(ASIO_ENABLE_HANDLER_TRACKING)
# if defined(ASIO_HAS_SOURCE_LOCATION)
          , const zasio::detail::source_location& location
# endif // defined(ASIO_HAS_SOURCE_LOCATION)
#endif // defined(ASIO_ENABLE_HANDLER_TRACKING)
        )
...
#if defined(ASIO_ENABLE_HANDLER_TRACKING)
# if defined(ASIO_HAS_SOURCE_LOCATION)
      zasio::detail::source_location location_;
# endif // defined(ASIO_HAS_SOURCE_LOCATION)
#endif // defined(ASIO_ENABLE_HANDLER_TRACKING)
    };
...
struct coroutine_traits<void, C&,
    zasio::experimental::detail::co_composed_state<
      Executors, Handler, Return>,
    Args...>
{
  using promise_type =
    zasio::experimental::detail::co_composed_promise<
      Executors, Handler, Return>;
};
