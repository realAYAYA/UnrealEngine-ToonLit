 ...
 * {
 *   return zasio::async_initiate<
 *     CompletionToken, void(std::error_code)>(
 *       zasio::experimental::co_composed(
 *         [](auto state, tcp::socket& socket) -> void
 *         {
 *           state.reset_cancellation_state(
 *             zasio::enable_terminal_cancellation());
 *
 ...
 *                 make_error_code(zasio::error::operation_aborted));
 *
 *             auto [e2, n2] =
 *               co_await zasio::async_write(socket,
 *                 zasio::buffer(data, n1),
 *                 zasio::as_tuple(zasio::deferred));
 *
 *             if (e2)
 ...
 *                     zasio::buffer(data), zasio::deferred);
 *
 *                 co_await zasio::async_write(socket,
 *                     zasio::buffer(data, n), zasio::deferred);
 *               }
 *             }
