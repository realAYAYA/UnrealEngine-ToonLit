...
kqueue_reactor::kqueue_reactor(zasio::execution_context& ctx)
    ...
    registered_descriptors_mutex_(mutex_.enabled())
{
  ...
  {
    zasio::error_code error(errno,
        zasio::error::get_system_category());
    zasio::detail::throw_error(error);
  }
}
  ...
  if (fork_ev == zasio::execution_context::fork_child)
  {
    ...
    if (::kevent(kqueue_fd_, events, 1, 0, 0, 0) == -1)
    {
      zasio::error_code ec(errno,
          zasio::error::get_system_category());
      zasio::detail::throw_error(ec, "kqueue interrupter registration");
    }
        ...
        state != 0; state = state->next_)
    {
      if (state->num_kevents_ > 0)
      {
        ...
        if (::kevent(kqueue_fd_, events, state->num_kevents_, 0, 0, 0) == -1)
        {
          zasio::error_code ec(errno,
              zasio::error::get_system_category());
          zasio::detail::throw_error(ec, "kqueue re-registration");
        }
      }
    }
  }
...
int kqueue_reactor::do_kqueue_create()
{
  ...
  if (fd == -1)
  {
    zasio::error_code ec(errno,
        zasio::error::get_system_category());
    zasio::detail::throw_error(ec, "kqueue");
  }
  return fd;
}
