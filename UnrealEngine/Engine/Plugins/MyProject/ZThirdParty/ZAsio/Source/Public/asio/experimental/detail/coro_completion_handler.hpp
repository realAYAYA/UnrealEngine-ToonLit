...
auto coro_interpret_result(
    std::tuple<zasio::error_code, Args...>&& args)
{
  if (std::get<0>(args))
    zasio::detail::throw_exception(
        zasio::system_error(std::get<0>(args)));
...
inline auto coro_interpret_result(
    std::tuple<zasio::error_code>&& args)
{
  if (std::get<0>(args))
    zasio::detail::throw_exception(
        zasio::system_error(std::get<0>(args)));
...
template <typename Arg>
auto coro_interpret_result(std::tuple<zasio::error_code, Arg>&& args)
{
  if (std::get<0>(args))
    zasio::detail::throw_exception(
        zasio::system_error(std::get<0>(args)));
