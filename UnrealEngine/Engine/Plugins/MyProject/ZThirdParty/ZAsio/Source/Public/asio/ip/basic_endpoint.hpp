...
/**
 * The zasio::ip::basic_endpoint class template describes an endpoint that
...
public:
...
#else
  typedef zasio::detail::socket_addr_type data_type;
#endif
   ...
   * @code
   * zasio::ip::tcp::endpoint ep(zasio::ip::tcp::v4(), 1234);
   * @endcode
   *
   * To specify an IPv6 UDP endpoint for port 9876, use:
   * @code
   * zasio::ip::udp::endpoint ep(zasio::ip::udp::v6(), 9876);
   * @endcode
   */
  ...
  basic_endpoint(const zasio::ip::address& addr,
      port_type port_num) ASIO_NOEXCEPT
    : impl_(addr, port_num)
  {
  }
  ...
  zasio::ip::address address() const ASIO_NOEXCEPT
  {
    return impl_.address();
  }

  /// Set the IP address associated with the endpoint.
  void address(const zasio::ip::address& addr) ASIO_NOEXCEPT
  {
    impl_.address(addr);
  }
...
private:
  ...
  zasio::ip::detail::endpoint impl_;
};
 ...
 *
 * @relates zasio::ip::basic_endpoint
 */
...
template <typename InternetProtocol>
struct hash<zasio::ip::basic_endpoint<InternetProtocol> >
{
  std::size_t operator()(
      const zasio::ip::basic_endpoint<InternetProtocol>& ep)
    const ASIO_NOEXCEPT
  {
    std::size_t hash1 = std::hash<zasio::ip::address>()(ep.address());
