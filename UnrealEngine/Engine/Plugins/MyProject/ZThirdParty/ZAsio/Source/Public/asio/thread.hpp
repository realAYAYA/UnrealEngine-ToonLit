...
/**
 * The zasio::thread class implements the smallest possible subset of the
 ...
 *
 * @par Example
 * A typical use of zasio::thread would be to launch a thread to run an
 * io_context's event processing loop:
 *
 * @par
 * @code zasio::io_context io_context;
 * // ...
 * zasio::thread t(boost::bind(&zasio::io_context::run, &io_context));
 * // ...
 * t.join(); @endcode
 */
class thread
