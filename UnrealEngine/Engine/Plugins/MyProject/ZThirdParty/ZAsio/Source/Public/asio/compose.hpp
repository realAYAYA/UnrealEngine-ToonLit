 ...
 * {
 *   tcp::socket& socket_;
 *   zasio::mutable_buffer buffer_;
 *   enum { starting, reading, writing } state_;
 *
 *   template <typename Self>
 *   void operator()(Self& self,
 *       zasio::error_code error = {},
 *       std::size_t n = 0)
 *   {
 *     switch (state_)
 *     {
 ...
 *       {
 *         state_ = writing;
 *         zasio::async_write(socket_, buffer_,
 *             zasio::transfer_exactly(n),
 *             std::move(self));
 *       }
 *       break;
 *     case writing:
 ...
 *   decltype(
 *     zasio::async_compose<CompletionToken,
 *       void(zasio::error_code, std::size_t)>(
 *         std::declval<async_echo_implementation>(),
 *         token, socket))
 * {
 *   return zasio::async_compose<CompletionToken,
 *     void(zasio::error_code, std::size_t)>(
