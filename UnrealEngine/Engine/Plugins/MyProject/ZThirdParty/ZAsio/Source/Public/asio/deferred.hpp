...
public:
  ...
  auto operator()(
    ...
    -> decltype(
        zasio::async_initiate<CompletionToken, Signatures...>(
          declval<initiate>(), token,
          ASIO_MOVE_OR_LVALUE(Head)(this->head_),
          ASIO_MOVE_OR_LVALUE(Tail)(this->tail_)))
  {
    return zasio::async_initiate<CompletionToken, Signatures...>(
        initiate(), token,
...
private:
  ...
  auto invoke_helper(
    ...
    -> decltype(
        zasio::async_initiate<CompletionToken, void(Values...)>(
          initiate(), token,
          std::get<I>(
            ASIO_MOVE_CAST(std::tuple<Values...>)(this->values_))...))
  {
    return zasio::async_initiate<CompletionToken, void(Values...)>(
        initiate(), token,
 ...
 * operation, typically using the special value @c zasio::deferred. For
 * example:
 *
 * @code auto my_deferred_op
 *   = my_socket.async_read_some(my_buffer,
 *       zasio::deferred); @endcode
 *
...
/**
 * See the documentation for zasio::deferred_t for a usage example.
 */
