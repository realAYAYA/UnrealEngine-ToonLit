...
public:
   ...
   * } @endcode
   */
  basic_yield_context operator[](zasio::error_code& ec) const
  {
 ...
 *
 * @code zasio::spawn(my_strand, do_echo, zasio::detached);
 *
 * // ...
 *
 * void do_echo(zasio::yield_context yield)
 * {
 *   try
 *   {
 *     char data[128];
 ...
 *           zasio::buffer(data), yield);
 *
 *       zasio::async_write(my_socket,
 *           zasio::buffer(data, length), yield);
 *     }
 *   }
 ...
 */
template <typename Function>
void spawn(const zasio::io_context::strand& s,
    ASIO_MOVE_ARG(Function) function,
    const boost::coroutines::attributes& attributes
      = boost::coroutines::attributes());
