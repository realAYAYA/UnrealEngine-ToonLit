...
dev_poll_reactor::dev_poll_reactor(zasio::execution_context& ctx)
  : zasio::detail::execution_context_service_base<dev_poll_reactor>(ctx),
...
void dev_poll_reactor::run(long usec, op_queue<operation>& ops)
{
  ...
  if (events_size > 0)
  {
    ...
    {
      zasio::error_code ec = zasio::error_code(
...
int dev_poll_reactor::do_dev_poll_create()
{
  ...
  if (fd == -1)
  {
    zasio::error_code ec(errno,
        zasio::error::get_system_category());
    zasio::detail::throw_error(ec, "/dev/poll");
  }
  return fd;
}
