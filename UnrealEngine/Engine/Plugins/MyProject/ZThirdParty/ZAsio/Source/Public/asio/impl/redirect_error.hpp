...
public:
  ...
  redirect_error_handler(zasio::error_code& ec,
  ...
  typename enable_if<
    !is_same<typename decay<Arg>::type, zasio::error_code>::value
  >::type
  ...
  void operator()(const zasio::error_code& ec,
...
struct redirect_error_signature<R(zasio::error_code, Args...)>
{
  typedef R type(Args...);
};

template <typename R, typename... Args>
struct redirect_error_signature<R(const zasio::error_code&, Args...)>
{
  typedef R type(Args...);
};
...
template <typename R, typename... Args>
struct redirect_error_signature<
  R(zasio::error_code, Args...) noexcept>
{
...
template <typename R, typename... Args>
struct redirect_error_signature<
  R(const zasio::error_code&, Args...) noexcept>
{
  typedef R type(Args...) & noexcept;
};
...
template <typename R>
struct redirect_error_signature<R(zasio::error_code)>
{
  typedef R type();
};

template <typename R>
struct redirect_error_signature<R(const zasio::error_code&)>
{
  typedef R type();
};
...
#define ASIO_PRIVATE_REDIRECT_ERROR_DEF(n) \
  ...
  struct redirect_error_signature< \
      R(zasio::error_code, ASIO_VARIADIC_TARGS(n))> \
  { \
  ...
  template <typename R, ASIO_VARIADIC_TPARAMS(n)> \
  struct redirect_error_signature< \
      R(const zasio::error_code&, ASIO_VARIADIC_TARGS(n))> \
  { \
    typedef R type(ASIO_VARIADIC_TARGS(n)); \
  }; \
  /**/
...
template <typename R>
struct redirect_error_signature<
  R(const zasio::error_code&) noexcept>
{
  typedef R type() noexcept;
};
...
#define ASIO_PRIVATE_REDIRECT_ERROR_DEF(n) \
  template <typename R, ASIO_VARIADIC_TPARAMS(n)> \
  struct redirect_error_signature< \
      R(zasio::error_code, ASIO_VARIADIC_TARGS(n)) noexcept> \
  { \
      ...
      typename detail::redirect_error_signature<Signature>::type>
{
  ...
  struct init_wrapper
  {
    explicit init_wrapper(zasio::error_code& ec)
      : ec_(ec)
    {
    }
