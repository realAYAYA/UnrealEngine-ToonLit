...
public:
  ...
  mutable_buffer(void* data, std::size_t size,
      zasio::detail::function<void()> debug_check)
    : data_(data),
      size_(size),
  ...
  const zasio::detail::function<void()>& get_debug_check() const
  {
    return debug_check_;
  }
#endif // ASIO_ENABLE_BUFFER_DEBUGGING
...
#if defined(ASIO_ENABLE_BUFFER_DEBUGGING)
  zasio::detail::function<void()> debug_check_;
#endif // ASIO_ENABLE_BUFFER_DEBUGGING
};
...
/** @defgroup buffer_sequence_begin zasio::buffer_sequence_begin
 *
 * @brief The zasio::buffer_sequence_begin function returns an iterator
...
/** @defgroup buffer_sequence_end zasio::buffer_sequence_end
 *
 * @brief The zasio::buffer_sequence_end function returns an iterator
 ...
 * auto i = zasio::buffer_sequence_begin(buffers);
 * auto end = zasio::buffer_sequence_end(buffers);
 * for (; i != end; ++i)
 * {
 *   const_buffer b(*i);
 *   total_size += b.size();
...
inline std::size_t buffer_size(const BufferSequence& b) ASIO_NOEXCEPT
{
  return detail::buffer_size(
      ...
      zasio::buffer_sequence_begin(b),
      zasio::buffer_sequence_end(b));
}

#if !defined(ASIO_NO_DEPRECATED)

/** @defgroup buffer_cast zasio::buffer_cast
 *
 * @brief (Deprecated: Use the @c data() member function.) The
 * zasio::buffer_cast function is used to obtain a pointer to the
 ...
 * @code zasio::const_buffer b1 = ...;
 * const unsigned char* p1 = zasio::buffer_cast<const unsigned char*>(b1);
 * @endcode
 *
 * To access the memory of a modifiable buffer, use:
 * @code zasio::mutable_buffer b2 = ...;
 * unsigned char* p2 = zasio::buffer_cast<unsigned char*>(b2);
 * @endcode
 *
 * The zasio::buffer_cast function permits violations of type safety, so
...
/** @defgroup buffer zasio::buffer
 *
 * @brief The zasio::buffer function is used to create a buffer object to
 ...
 * specified size:
 *
 * @code sock.send(zasio::buffer(data, size)); @endcode
 *
 * In the above example, the return value of zasio::buffer meets the
 ...
 *
 * @code char d1[128];
 * size_t bytes_transferred = sock.receive(zasio::buffer(d1));
 *
 * std::vector<char> d2(128);
 * bytes_transferred = sock.receive(zasio::buffer(d2));
 *
 * std::array<char, 128> d3;
 * bytes_transferred = sock.receive(zasio::buffer(d3));
 *
 * boost::array<char, 128> d4;
 * bytes_transferred = sock.receive(zasio::buffer(d4)); @endcode
 *
 ...
 *
 * vector<unsigned char> data(zasio::buffer_size(buffers));
 * zasio::buffer_copy(zasio::buffer(data), buffers); @endcode
 *
 ...
 *
 * For the zasio::buffer overloads that accept an argument of type
 ...
 *
 * @code b1 = zasio::buffer(a); @endcode
 *
 * represents the entire array, <tt>{ 'a', 'b', 'c', 'd', 'e' }</tt>. An
 * optional second argument to the zasio::buffer function may be used to
 * limit the size, in bytes, of the buffer:
 *
 * @code b2 = zasio::buffer(a, 3); @endcode
 *
 ...
 *
 * @code b4 = zasio::buffer(b1 + 1, 3); @endcode
 *
 ...
 * bufs2.push_back(zasio::buffer(d1));
 * bufs2.push_back(zasio::buffer(d2));
 * bufs2.push_back(zasio::buffer(d3));
 * bytes_transferred = sock.send(bufs2); @endcode
 *
 * @par Buffer Literals
 *
 * The `_buf` literal suffix, defined in namespace
 * <tt>zasio::buffer_literals</tt>, may be used to create @c const_buffer
 * objects from string, binary integer, and hexadecimal integer literals.
 * For example:
 *
 * @code
 * using namespace my_zasio::buffer_literals;
 *
 * zasio::const_buffer b1 = "hello"_buf;
 * zasio::const_buffer b2 = 0xdeadbeef_buf;
 * zasio::const_buffer b3 = 0x0123456789abcdef0123456789abcdef_buf;
 * zasio::const_buffer b4 = 0b1010101011001100_buf; @endcode
 *
...
{
  typedef typename zasio::detail::buffer_types<PodType>::buffer_type
    buffer_type;
  typedef typename zasio::detail::buffer_types<PodType>::container_type
    container_type;
...
public:
  ...
  {
    return const_buffers_type(zasio::buffer(string_, size_));
  }
#endif // !defined(ASIO_NO_DYNAMIC_BUFFER_V1)
  ...
  {
    return mutable_buffers_type(zasio::buffer(
          zasio::buffer(string_, max_size_) + pos, n));
  }
  ...
  {
    return const_buffers_type(zasio::buffer(
          zasio::buffer(string_, max_size_) + pos, n));
  }
  ...
  mutable_buffers_type prepare(std::size_t n)
  {
    if (size() > max_size() || max_size() - size() < n)
    {
      ...
      zasio::detail::throw_exception(ex);
    }
    ...
    return zasio::buffer(zasio::buffer(string_) + size_, n);
  }
...
public:
  ...
  const_buffers_type data() const ASIO_NOEXCEPT
  {
    return const_buffers_type(zasio::buffer(vector_, size_));
  }
#endif // !defined(ASIO_NO_DYNAMIC_BUFFER_V1)
  ...
  {
    return mutable_buffers_type(zasio::buffer(
          zasio::buffer(vector_, max_size_) + pos, n));
  }
  ...
  mutable_buffers_type prepare(std::size_t n)
  {
    ...
    return zasio::buffer(zasio::buffer(vector_) + size_, n);
  }
...
/** @defgroup dynamic_buffer zasio::dynamic_buffer
 *
 * @brief The zasio::dynamic_buffer function is used to create a
...
/** @defgroup buffer_copy zasio::buffer_copy
 *
 * @brief The zasio::buffer_copy function is used to copy bytes from a
    ...
    std::size_t max_bytes_to_copy) ASIO_NOEXCEPT
{
  return (buffer_copy_1)(*target_begin,
      zasio::buffer(*source_begin, max_bytes_to_copy));
}
      ...
      = (std::numeric_limits<std::size_t>::max)()) ASIO_NOEXCEPT
{
  ...
  for (mutable_buffer target_buffer(
        zasio::buffer(*target_begin, max_bytes_to_copy));
    ...
    std::size_t max_bytes_to_copy) ASIO_NOEXCEPT
{
      ...
      && target_iter != target_end && source_iter != source_end)
  {
    ...
    std::size_t bytes_copied = (buffer_copy_1)(
        target_buffer, zasio::buffer(source_buffer,
    ...
    const ConstBufferSequence& source) ASIO_NOEXCEPT
{
  return detail::buffer_copy(
      ...
      zasio::buffer_sequence_begin(target),
      zasio::buffer_sequence_end(target),
      zasio::buffer_sequence_begin(source),
      zasio::buffer_sequence_end(source));
}
    ...
    std::size_t max_bytes_to_copy) ASIO_NOEXCEPT
{
  return detail::buffer_copy(
      ...
      zasio::buffer_sequence_end(target),
      zasio::buffer_sequence_begin(source),
      zasio::buffer_sequence_end(source), max_bytes_to_copy);
}
...
struct is_mutable_buffer_sequence
  ...
  : zasio::detail::is_buffer_sequence<T, mutable_buffer>
...
struct is_const_buffer_sequence
  ...
  : integral_constant<bool, automatically_determined>
#else // defined(GENERATING_DOCUMENTATION)
  : zasio::detail::is_buffer_sequence<T, const_buffer>
#endif // defined(GENERATING_DOCUMENTATION)
{
};
...
struct is_dynamic_buffer_v1
  ...
  : integral_constant<bool, automatically_determined>
#else // defined(GENERATING_DOCUMENTATION)
  : zasio::detail::is_dynamic_buffer_v1<T>
#endif // defined(GENERATING_DOCUMENTATION)
{
};
#endif // !defined(ASIO_NO_DYNAMIC_BUFFER_V1)
...
struct is_dynamic_buffer_v2
  ...
  : integral_constant<bool, automatically_determined>
#else // defined(GENERATING_DOCUMENTATION)
  : zasio::detail::is_dynamic_buffer_v2<T>
#endif // defined(GENERATING_DOCUMENTATION)
{
};
...
struct is_dynamic_buffer
  ...
  : zasio::is_dynamic_buffer_v2<T>
#else // defined(ASIO_NO_DYNAMIC_BUFFER_V1)
  : zasio::is_dynamic_buffer_v1<T>
#endif // defined(ASIO_NO_DYNAMIC_BUFFER_V1)
{
};
