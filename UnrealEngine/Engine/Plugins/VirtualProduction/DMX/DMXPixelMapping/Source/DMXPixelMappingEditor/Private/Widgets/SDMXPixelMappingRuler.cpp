...
#define LOCTEXT_NAMESPACE "SDMXPixelMappingRuler"
...
struct SDMXPixelMappingRuler::FScrubRangeToScreen
{
	...
	float LocalXToInput(float ScreenX) const
	{
		...
		float PixelsPerInput_Local = FMath::IsNearlyZero(PixelsPerInput) ? KINDA_SMALL_NUMBER : PixelsPerInput; // prevent devide by zero errors
		return ( LocalX / PixelsPerInput_Local ) + ViewInput.GetLowerBoundValue();
	}
...
struct SDMXPixelMappingRuler::FDrawTickArgs
{
...
/////////////////////////////////////////////////////
// SDMXPixelMappingRuler

void SDMXPixelMappingRuler::Construct(const SDMXPixelMappingRuler::FArguments& InArgs)
{
...
float SDMXPixelMappingRuler::DetermineOptimalSpacing(float InPixelsPerInput, uint32 MinTick, float MinTickSpacing) const
{
...
void SDMXPixelMappingRuler::SetRuling(FVector2D InAbsoluteOrigin, float InSlateToUnitScale)
{
	AbsoluteOrigin = InAbsoluteOrigin;
	SlateToUnitScale = InSlateToUnitScale;
}

void SDMXPixelMappingRuler::SetCursor(TOptional<FVector2D> InAbsoluteCursor)
{
	AbsoluteCursor = InAbsoluteCursor;
}

int32 SDMXPixelMappingRuler::DrawTicks( FSlateWindowElementList& OutDrawElements, const struct FScrubRangeToScreen& RangeToScreen, FDrawTickArgs& InArgs ) const
{
...
int32 SDMXPixelMappingRuler::OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
...
FReply SDMXPixelMappingRuler::OnMouseButtonDown( const FGeometry& MyGeometry, const FPointerEvent& MouseEvent )
{
...
FReply SDMXPixelMappingRuler::OnMouseButtonUp(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	return FReply::Unhandled();
}

FReply SDMXPixelMappingRuler::OnMouseMove(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	return FReply::Unhandled();
}

FVector2D SDMXPixelMappingRuler::ComputeDesiredSize( float ) const
{
	return Orientation == Orient_Horizontal ? FVector2D(100, 18) : FVector2D(18, 100);
}

FReply SDMXPixelMappingRuler::OnMouseWheel(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	return FReply::Unhandled();
}
