...
bool FNetworkNode::SendMessage(const TArray<uint8>& Message, double Timeout_s)
{
	if (bWriteError)
	{
		UE_LOG(LogDatasmithDispatcher, Error, TEXT("bWriteError flag raised, can't write"));
		return false;
	}
	...
	{
		UE_LOG(LogDatasmithDispatcher, Error, TEXT("connection state error"));
		return false;
	}
	...
	if (!bCanWrite)
	{
		UE_LOG(LogDatasmithDispatcher, Warning, TEXT("can't write on socket"));
		return false;
	}
...
bool FNetworkNode::ReceiveMessage(TArray<uint8>& OutMessage, double Timeout_s)
{
	...
	if (IncommingMessage.Header.ByteSize < 0)
	{
		...
		{
			UE_LOG(LogDatasmithDispatcher, Error, TEXT("Parsed header failed"));
			bReadError = true;
			return false;
		}
	...
	while (IncommingMessage.Header.ByteSize >= 0 && ConnectedSocket->HasPendingData(PendingByteCount))
	{
		...
		{
			bReadError = true;
			UE_LOG(LogDatasmithDispatcher, Error, TEXT("Recv issue"));
			return false;
		}
...
FSocket* FNetworkNode::CreateInternalSocket(const FString& Description)
{
	...
	UE_CLOG(!NewSocket, LogDatasmithDispatcher, Error, TEXT("Socket creation failure"));
	return NewSocket;
}
...
{
	ListeningSocket = CreateInternalSocket(TEXT("datasmith listening socket"));
	...
	if (BoundPort == 0)
	{
		...
		UE_LOG(LogDatasmithDispatcher, Error, TEXT("Socket binding failure"));
		return;
	}
	...
	if (!ListeningSocket->Listen(0))
	{
		...
		UE_LOG(LogDatasmithDispatcher, Error, TEXT("Socket listen failure"));
		return;
	}
}
...
bool FNetworkClientNode::Connect(const FString& Description, int32 ServerPort, double Timeout_s)
{
	...
	UE_LOG(LogDatasmithDispatcher, Error, TEXT("Client socket failed to connect"));
	CloseSocket(ConnectedSocket);
	return false;
}
