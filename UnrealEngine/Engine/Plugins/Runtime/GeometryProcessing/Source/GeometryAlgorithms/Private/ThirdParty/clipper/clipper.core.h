...
{
	return TransformPath<double, int64>(path);
}

inline PathsD Paths64ToPathsD(const Paths64& paths)
{
	return TransformPaths<double, int64>(paths);
}

inline Path64 PathDToPath64(const PathD& path)
{
	return TransformPath<int64, double>(path);
}

inline Paths64 PathsDToPaths64(const PathsD& paths)
{
	return TransformPaths<int64, double>(paths);
}
...
inline double DistanceFromLineSqrd(const Point<T>& pt, const Point<T>& ln1, const Point<T>& ln2)
{
	...
	double C = A * static_cast<double>(ln1.x) + B * static_cast<double>(ln1.y);
	C = A * static_cast<double>(pt.x) + B * static_cast<double>(pt.y) - C;
	// @UE END
	return (C * C) / (A * A + B * B);
}
