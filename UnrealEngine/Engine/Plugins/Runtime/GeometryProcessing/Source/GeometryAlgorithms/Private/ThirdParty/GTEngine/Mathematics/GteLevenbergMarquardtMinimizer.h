...
namespace gte
{
    ...
    public:
        ...
        typedef GVector<Real> DVector;  // NumPDimensions elements
        typedef GVector<Real> RVector;  // NumFDImensions elements
        typedef GMatrix<Real> JMatrix;  // NumFDimensions-by-NumPDimensions elements
        typedef GMatrix<Real> JTJMatrix;  // NumPDimensions-by-NumPDimensions elements
        typedef GVector<Real> JTFVector;  // NumPDimensions elements
        ...
        // NOTE: The C++ compiler for Microsoft Visual Studio 12.0.21005.1 REL
        // (MSVS 2013) appears to have a bug regarding passing std::function
        // arguments.  The unit-test code for the first LevenbergMarquardtMinimizer
        // constructor was passed std::function objects of type FFunction and
        // JFunction.  The compiler claimed that the constructor call is
        // ambiguous because the JFunction portion of the first constructor's
        // signature matches the JPlusFunction portion of the second
        // constructor's signature, which is clearly not correct.  When using
        // MSVS 2013, you need to explicitly typecast using
        //   LevenbergMarquardtMinimizer<Real> minimizer(
        //       numPDimensions, numFDimensions,
        //       (LevenbergMarquardtMinimizer<Real>::FFunction const&) myFFunction,
        //       (LevenbergMarquardtMinimizer<Real>::JFunction const&) myJFunction);
        // The same typecasting is also required for the second constructor.
