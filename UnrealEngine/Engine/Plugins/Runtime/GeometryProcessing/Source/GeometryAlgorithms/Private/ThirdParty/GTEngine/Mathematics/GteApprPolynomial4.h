...
#include <ThirdParty/GTEngine/LowLevel/GteArray2.h>
...
Real ApprPolynomial4<Real>::Evaluate(Real x, Real y, Real z) const
{
    ...
    for (i2 = 0; i2 <= mZDegree; ++i2)
    {
        for (i1 = 0; i1 <= mYDegree; ++i1)
        {
			...
			// @todo original line here did no assignment, is this correct? Assume will be fixed in future versions of GTEngine.
