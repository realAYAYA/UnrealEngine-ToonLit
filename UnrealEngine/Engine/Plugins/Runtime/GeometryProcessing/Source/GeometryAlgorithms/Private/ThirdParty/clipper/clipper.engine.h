...
namespace Clipper2Lib {
	...
	using PolyPath64 = PolyPath<int64>;
	using PolyPathD = PolyPath<double>;

	template <typename T>
	using PolyTree = PolyPath<T>;
	using PolyTree64 = PolyTree<int64>;
	...
	struct Active {
		Point64 bot;
		Point64 top;
		int64 curr_x = 0;		//current (updated at every new scanline)
		double dx = 0.0;
	...
	private:
		...
		std::priority_queue<int64> scanline_list_;
		...
		void Reset();
		void InsertScanline(int64 y);
		bool PopScanline(int64 &y);
		bool PopLocalMinima(int64 y, LocalMinima *&local_minima);
		...
		virtual void InsertLocalMinimaIntoAEL(int64 bot_y);
		...
		inline void AdjustCurrXAndCopyToSEL(const int64 top_y);
		void DoIntersections(const int64 top_y);
		void AddNewIntersectNode(Active &e1, Active &e2, const int64 top_y);
		bool BuildIntersectList(const int64 top_y);
		...
		bool ResetHorzDirection(const Active &horz, const Active *max_pair,
			int64 &horz_left, int64 &horz_right);
		void DoTopOfScanbeam(const int64 top_y);
	...
	public:
		...
		{
			AddPaths(ScalePaths<int64, double>(subjects, scale_), PathType::Subject, false);
		}

		void AddOpenSubject(const PathsD& open_subjects)
		{
			AddPaths(ScalePaths<int64, double>(open_subjects, scale_), PathType::Subject, true);
		}

		void AddClip(const PathsD& clips)
		{
			AddPaths(ScalePaths<int64, double>(clips, scale_), PathType::Clip, false);
		}
		...
		bool Execute(ClipType clip_type, FillRule fill_rule, PathsD& closed_paths)
		{
			...
			closed_paths = ScalePaths<double, int64>(closed_paths64, invScale_);
			return true;
		}
			...
			FillRule fill_rule, PathsD& closed_paths, PathsD& open_paths)
		{
			...
			closed_paths = ScalePaths<double, int64>(closed_paths64, invScale_);
			open_paths = ScalePaths<double, int64>(open_paths64, invScale_);
			return true;
		}
