...
void floatTetWild::get_all_edges(const Mesh& mesh, std::vector<std::array<int, 2>>& edges){
    ...
    checkSlow(edges_tbb.size() == edges.size());
...
bool floatTetWild::is_boundary_edge(const Mesh& mesh, int v1_id, int v2_id, const AABBWrapper& tree) {
    ...
    ps.push_back(mesh.tet_vertices[v1_id].pos);
    int p0_id = 0;
    Scalar l = get_edge_length(mesh, v1_id, v2_id);
    int N = l / mesh.params.dd + 1;
    ps.push_back(mesh.tet_vertices[v2_id].pos);
    int p1_id = ps.size() - 1;
...
void floatTetWild::sample_triangle(const std::array<Vector3, 3>& vs, std::vector<Vector3>& ps, Scalar sampling_dist) {
	...
	Vector3 v0 = vs[max_i];
	Vector3 v1 = vs[mod3(max_i + 1)];
	Vector3 v2 = vs[mod3(max_i + 2)];
	
	auto normalize = [](const Vector3& A)->Vector3
	{
		return A.normalized();
	};
	auto length = [](const Vector3& A)->Scalar
	{
		return A.norm();
	};
	auto distance = [](const Vector3& A, const Vector3& B) -> Scalar
	{
		return (A - B).norm();
	};
	auto dot = [](const Vector3& A, const Vector3& B) -> Scalar
	{
		return A.dot(B);
	};
    ...
    Vector3 n_v0v1 = normalize(v1 - v0);
    for (int n = 0; n <= N; n++) {
    ...
    Vector3 n_v0v2 = normalize(v2 - v0);
    Vector3 n_v1v2 = normalize(v2 - v1);
	...
	for (int m = 1; m <= M; m++) {
		...
		Vector3 v0_m = v0 + m * sqrt3_2 * sampling_dist / sin_v0 * n_v0v2;
		Vector3 v1_m = v1 + m * sqrt3_2 * sampling_dist / sin_v1 * n_v1v2;
		if (distance(v0_m, v1_m) <= sampling_dist)
			break;

		Scalar delta_d = ((n + (m % 2) / 2.0) - m * sqrt3_2 / tan_v0) * sampling_dist;
		Vector3 v = v0_m + delta_d * n_v0v1;
	...
	if (N > 1) {
		if (N == int(N))
			N -= 1;
		Vector3 n_v1v2_ = normalize(v2 - v1);
		for (int n = 1; n <= N; n++) {
	...
	if (N > 1) {
		if (N == int(N))
			N -= 1;
		Vector3 n_v2v0 = normalize(v0 - v2);
		for (int n = 1; n <= N; n++) {
...
bool floatTetWild::sample_triangle_and_check_is_out(const std::array<Vector3, 3>& vs, Scalar sampling_dist,
        Scalar eps_2, const AABBWrapper& tree, FMeshIndex& prev_facet){
	...
	Vector3 n_v0v1 = (v1 - v0).normalized();
    for (int n = 0; n <= N; n++) {
    ...
    if (N > 1) {
        ...
        Vector3 n_v1v2_ = normalize(v2 - v1);
        for (int n = 1; n <= N; n++) {
            if (tree.is_out_sf_envelope(v1 + n_v1v2_ * sampling_dist * n, eps_2, prev_facet, sq_dist, nearest_point))
                return true;
        }
    }
...
void floatTetWild::set_intersection(const std::unordered_set<int>& s1, const std::unordered_set<int>& s2, const std::unordered_set<int>& s3,
    ...
    checkSlow(s1.size() <= s2.size());
    checkSlow(s1.size() <= s3.size());
...
Scalar floatTetWild::AMIPS_energy(const std::array<Scalar, 12>& T) {
	...
	constexpr bool bUseOldEnergy = false;
    if(bUseOldEnergy)
	{
        return res;
    }
    ...
    if (res > 1e8) {
        if(is_degenerate(Vector3(T[0], T[1], T[2]), Vector3(T[3], T[4], T[5]), Vector3(T[6], T[7], T[8]),
            ...
            ensureMsgf(false, TEXT("Energy computation degenerate found"));
            return std::numeric_limits<double>::infinity();
        }

        // Note: Storage of 100 chosen as a conservative value that does not appear to be exceeded in practice, optimal could be smaller
		// (but evaluating this does not seem to be a bottleneck)
		using gteExact = gte::BSRational<gte::UIntegerFP32<100>>;
		std::array<gteExact, 12> r_T;

        for (int j = 0; j < 12; j++)
            r_T[j] = T[j];
        const gteExact twothird = gteExact(2) / gteExact(3);
		gteExact tmp = ((-r_T[1 + 2] + r_T[1 + 5]) * r_T[1 + 1] + r_T[1 + 2] * r_T[1 + 7] +
		...
		auto res_r = (gteExact(27) / gteExact(16)) * to_pow3 * to_pow3 * to_pow3 / (tmp * tmp);
		double tocbrt = double(res_r);
        return std::cbrt(tocbrt);
    } else {
        return res;
    }
}
