...
// File Version: 3.10.1 (2017/09/16)
...
template <typename Real>
OBBTreeForPoints<Real>::OBBTreeForPoints(uint32_t numPoints, char const* points, size_t stride, uint32_t height)
    :
    ...
    mPartition(numPoints)
{
    ...
    {
        uint32_t minPowerOfTwo = (uint32_t)RoundUpToPowerOfTwo(mNumPoints);
        mHeight = Log2OfPowerOfTwo(minPowerOfTwo);
        numNodes = 2 * mNumPoints - 1;
    }
    else
    {
        // The maximum level cannot exceed 30 because we are storing the
        // indices into the node array as 32-bit unsigned integers.
        if (mHeight < 32)
        {
    ...
    // The tree is built recursively.  A reference to a Node is passed
    // to BuildTree and nodes are appended to a std::vector.  Because the
    // references are on the stack, we must guarantee no reallocations
    // to avoid invalidating the references.  TODO: This design can be
    // modified to pass indices to the nodes so that reallocation is not
    // a problem.
...
template <typename Real>
inline uint32_t OBBTreeForPoints<Real>::GetNumPoints() const
{
    return mNumPoints;
}

template <typename Real>
inline char const* OBBTreeForPoints<Real>::GetPoints() const
{
    return mPoints;
}

template <typename Real>
inline size_t OBBTreeForPoints<Real>::GetStride() const
{
    return mStride;
}

template <typename Real>
inline std::vector<typename OBBTreeForPoints<Real>::Node> const& OBBTreeForPoints<Real>::GetTree() const
{
    return mTree;
}

template <typename Real>
inline uint32_t OBBTreeForPoints<Real>::GetHeight() const
{
    return mHeight;
}

template <typename Real>
inline std::vector<uint32_t> const& OBBTreeForPoints<Real>::GetPartition() const
{
    return mPartition;
}

template <typename Real>
inline Vector3<Real> OBBTreeForPoints<Real>::GetPosition(uint32_t index) const
{
    return *reinterpret_cast<Vector3<Real> const*>(mPoints + index * mStride);
}

template <typename Real>
void OBBTreeForPoints<Real>::BuildTree(Node& node, uint32_t i0, uint32_t i1)
{
    node.minIndex = i0;
    node.maxIndex = i1;
    ...
    if (i0 == i1)
    {
        // We are at a leaf node.  The left and right child indices were
        // set to std::numeric_limits<uint32_t>::max() during construction.
    ...
    {
        // We are at an interior node.  Compute an oriented bounding box.
        ...
        // splitting axis.  Partition the points into two subsets, one for
        // the left child and one for the right child. The subsets have
        // numbers of elements that differ by at most 1, so the tree is
        // balanced.
...
template <typename Real>
void OBBTreeForPoints<Real>::ComputeOBB(uint32_t i0, uint32_t i1, OrientedBox3<Real>& box)
{
    ...
    // Solve the eigensystem and use the eigenvectors for the box axes.
...
template <typename Real>
void OBBTreeForPoints<Real>::SplitPoints(uint32_t i0, uint32_t i1, uint32_t& j0, uint32_t& j1,
    Vector3<Real> const& origin, Vector3<Real> const& direction)
{
...
template <typename Real>
OBBTreeForPoints<Real>::Node::Node()
    :
    depth(0),
...
template <typename Real>
OBBTreeForPoints<Real>::ProjectionInfo::ProjectionInfo()
    :
    pointIndex(0),
    projection((Real)0)
{
...
template <typename Real>
bool OBBTreeForPoints<Real>::ProjectionInfo::operator< (ProjectionInfo const& info) const
{
