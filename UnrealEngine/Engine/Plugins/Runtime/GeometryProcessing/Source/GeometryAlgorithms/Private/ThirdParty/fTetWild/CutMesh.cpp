...
#include "ThirdParty/fTetWild/CutMesh.h"
            ...
            for (int j = 0; j < 4; j++) {
                if (opp_t_ids.back()[j] < 0) {
                    if (t[(j + 1) % 4] < old_v_ids_size && t[(j + 2) % 4] < old_v_ids_size
                        ...
                        if(tmp.size() == 1)//A triangle just fills a v-shaped gap, all points are in v_ids, but the triangle is not in cut_mesh
                            continue;
...
void floatTetWild::CutMesh::expand_new(std::vector<int> &cut_t_ids) {
    const int _t = get_t(p_vs[0], p_vs[1], p_vs[2]);
    const std::array<Vector2, 3> tri_2d = {{to_2d(p_vs[0], _t), to_2d(p_vs[1], _t), to_2d(p_vs[2], _t)}};
...
bool floatTetWild::CutMesh::checkMesh() {
    return true;
    ...
    for (auto &m:map_v_ids) {
        ...
        if (std::fabs(dist) < mesh.params.eps_coplanar && to_plane_dists[lv_id] != 0 && !is_snapped[lv_id]) {
			UE_LOG(LogTemp, Warning, TEXT("Bad vertex in cut mesh"));
            is_good = false;
        }
    }
    return is_good;
