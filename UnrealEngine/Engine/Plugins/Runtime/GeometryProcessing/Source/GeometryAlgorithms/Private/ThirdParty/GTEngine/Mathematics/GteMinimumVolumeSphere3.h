...
namespace gte
{
    ...
    public:
        ...
        bool operator()(int numPoints, Vector3<InputType> const* points, Sphere3<InputType>& minimal)
        {
            if (numPoints >= 1 && points)
            {
                ...
                // time the sphere needs updating.  Linus Kallberg (Computer
                    ...
                    if (!SupportContains(i))
                    {
                        if (!Contains(i, ctMinimal))
                        {
                            ...
                            else
                            {
                                ...
                                LogWarning("ComputeType is not exact and failure occurred. Returning non-minimal sphere.");
    ...
    private:
        ...
        Sphere3<ComputeType> ExactSphere4(int i0, int i1, int i2, int i3) const
        {
            ...
            // in barycentric coordinates is C = x0*P0 + x1*P1 + x2*P2 + x3*P3,
