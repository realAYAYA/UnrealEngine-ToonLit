...
namespace Clipper2Lib {
	...
	struct Scanline {
		...
		explicit Scanline(int64 y_) : y(y_) {}
	};
	...
	inline int64 TopX(const Active& ae, const int64 currentY)
	{
		if ((currentY == ae.top.y) || (ae.top.x == ae.bot.x)) return ae.top.x;
		else if (currentY == ae.bot.y) return ae.bot.x;		
		// @UE BEGIN
		// explicit casts
		else return ae.bot.x + static_cast<int64>(std::round(ae.dx * static_cast<double>(currentY - ae.bot.y)));
		// @UE END
	}
	...
	Point64 GetIntersectPoint(const Active& e1, const Active& e2)
	{
		...
		if (e1.dx == 0)
		{
			...
			// @UE BEGIN
			// explicit casts
			b2 = static_cast<double>(e2.bot.y) - (static_cast<double>(e2.bot.x) / e2.dx);
			return Point64(e1.bot.x,
				static_cast<int64>(std::round(static_cast<double>(e1.bot.x) / e2.dx + b2)));
			// @UE END
		}
		else if (e2.dx == 0)
		{
			...
			// @UE BEGIN
			// explicit casts
			b1 = static_cast<double>(e1.bot.y) - (static_cast<double>(e1.bot.x) / e1.dx);
			return Point64(e2.bot.x,
				static_cast<int64>(std::round(static_cast<double>(e2.bot.x) / e1.dx + b1)));
			// @UE END
		}
		else
		{
			// @UE BEGIN
			// explicit casts
			b1 = static_cast<double>(e1.bot.x) - static_cast<double>(e1.bot.y) * e1.dx;
			b2 = static_cast<double>(e2.bot.x) - static_cast<double>(e2.bot.y) * e2.dx;
			double q = (b2 - b1) / (e1.dx - e2.dx);
			return (abs(e1.dx) < abs(e2.dx)) ?
				Point64(static_cast<int64>(std::round(e1.dx * q + b1)),
					static_cast<int64>(std::round(q))) :
				Point64(static_cast<int64>(std::round(e2.dx * q + b2)),
					static_cast<int64>(std::round(q)));
			// @UE END
		}
	}
		...
		const Point64& ln2a, const Point64& ln2b, PointD& ip)
	{
		...
		if (ln1b.x == ln1a.x)
		{
			...
			b2 = static_cast<double>(ln2a.y) - m2 * static_cast<double>(ln2a.x);
			ip.x = static_cast<double>(ln1a.x);
			ip.y = m2 * static_cast<double>(ln1a.x) + b2;
		}
		else if (ln2b.x == ln2a.x)
		{
			m1 = static_cast<double>(ln1b.y - ln1a.y) /
				static_cast<double>(ln1b.x - ln1a.x);
			b1 = static_cast<double>(ln1a.y) - m1 * static_cast<double>(ln1a.x);
			ip.x = static_cast<double>(ln2a.x);
			ip.y = m1 * static_cast<double>(ln2a.x) + b1;
		}
		else
		{
			m1 = static_cast<double>(ln1b.y - ln1a.y) /
	...
	void ClipperBase::CleanUp()
	{
		...
		scanline_list_ = std::priority_queue<int64>();
	...
	inline void ClipperBase::InsertScanline(int64 y)
	{
		scanline_list_.push(y);
	}


	bool ClipperBase::PopScanline(int64& y)
	{
	...
	bool ClipperBase::PopLocalMinima(int64 y, LocalMinima*& local_minima)
	{
	...
	void ClipperBase::InsertLocalMinimaIntoAEL(int64 bot_y)
	{
		...
		const Point64& seg2a, const Point64& seg2b)
	{
		...
		// @UE BEGIN
		// explicit casts
		return (((dy1 * static_cast<double>(seg2a.x - seg1a.x) - dx1 * static_cast<double>(seg2a.y - seg1a.y)) *
			(dy1 * static_cast<double>(seg2b.x - seg1a.x) - dx1 * static_cast<double>(seg2b.y - seg1a.y)) < 0) &&
			((dy2 * static_cast<double>(seg1a.x - seg2a.x) - dx2 * static_cast<double>(seg1a.y - seg2a.y)) *
				(dy2 * static_cast<double>(seg1b.x - seg2a.x) - dx2 * static_cast<double>(seg1b.y - seg2a.y)) < 0));
		// @UE END
	}
	...
	inline void ClipperBase::AdjustCurrXAndCopyToSEL(const int64 top_y)
	{
		Active* e = actives_;
		sel_ = e;
		while (e)
	...
	void ClipperBase::DoIntersections(const int64 top_y)
	{
	...
	void ClipperBase::AddNewIntersectNode(Active& e1, Active& e2, int64 top_y)
	{
	...
	bool ClipperBase::BuildIntersectList(const int64 top_y)
	{
	...
	bool ClipperBase::ResetHorzDirection(const Active& horz,
		const Active* max_pair, int64& horz_left, int64& horz_right)
	{
	...
	void ClipperBase::DoTopOfScanbeam(const int64 y)
	{
	...
	inline bool HorzEdgesOverlap(int64 x1a, int64 x1b, int64 x2a, int64 x2b)
	{
		const int64 minOverlap = 2;
	...
	inline bool ValueBetween(int64 val, int64 end1, int64 end2)
	{
	...
	inline bool ValueEqualOrBetween(int64 val, int64 end1, int64 end2)
	{
