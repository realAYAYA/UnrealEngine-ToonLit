...
template <typename Real>
void DCPQuery<Real, Triangle3<Real>, AlignedBox3<Real>>::SetMaxLCPIterations(int maxLCPIterations)
{
    mLCP.SetMaxIterations(maxLCPIterations);
}

template <typename Real>
typename DCPQuery<Real, Triangle3<Real>, AlignedBox3<Real>>::Result
DCPQuery<Real, Triangle3<Real>, AlignedBox3<Real>>::operator()(
    Triangle3<Real> const& triangle, AlignedBox3<Real> const& box)
{
    Result result;

    // Translate the triangle and aligned box so that the aligned box becomes
    // a canonical box.
