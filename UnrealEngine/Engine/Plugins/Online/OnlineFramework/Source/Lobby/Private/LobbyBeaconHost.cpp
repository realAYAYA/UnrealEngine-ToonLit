...
{
	UE_LOG(LogLobbyBeacon, Log, TEXT("KickPlayer for %s. IsValid %d UNetConnection %s UNetDriver %s State %d"), *GetNameSafe(ClientActor), IsValid(ClientActor), *GetNameSafe(ClientActor->BeaconConnection), ClientActor->BeaconConnection ? *GetNameSafe(ClientActor->BeaconConnection->Driver) : TEXT("null"), ClientActor->BeaconConnection ? ClientActor->BeaconConnection->GetConnectionState() : -1);
