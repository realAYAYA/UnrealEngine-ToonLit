...
#if WITH_EDITORONLY_DATA
void UNiagaraDataInterfaceColorCurve::GetFunctionsInternal(TArray<FNiagaraFunctionSignature>& OutFunctions) const
{
...
void UNiagaraDataInterfaceColorCurve::SyncCurvesToAsset()
{
	...
	else if (UCurveVector* VecCurve = Cast<UCurveVector>(CurveAsset))
	{
		...
		BlueCurve = VecCurve->FloatCurves[2];
		AlphaCurve.Reset();
	...
	else if (UCurveFloat* FloatCurve = Cast<UCurveFloat>(CurveAsset))
	{
		...
		BlueCurve = FloatCurve->FloatCurve;
		AlphaCurve = FloatCurve->FloatCurve;
	}
}
