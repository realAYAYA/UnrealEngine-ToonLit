...
void SAvaMultiComboButton::Construct( const FArguments& InArgs )
{
	...
	SMenuAnchor::Construct( SMenuAnchor::FArguments()
		...
		[
			...
			.ToolTipText( this, &SAvaMultiComboButton::GetFilteredToolTipText, InArgs._ToolTipText)
...
FText SAvaMultiComboButton::GetFilteredToolTipText(TAttribute<FText> ToolTipText) const
{
	if (IsOpen())
	{
...
FReply SAvaMultiComboButton::OnButtonClicked()
{
...
FReply SAvaMultiComboButton::OnKeyDown(const FGeometry& MyGeometry, const FKeyEvent& InKeyEvent)
{
	FReply Reply = FReply::Unhandled();

	if (FSlateApplication::Get().GetNavigationActionFromKey(InKeyEvent) == EUINavigationAction::Accept)
	{
		// Handle click button with controller.
		ButtonPtr->SimulateClick();
...
void SAvaMultiComboButton::SetMenuContent(TSharedRef<SWidget> InContent)
{
...
void SAvaMultiComboButton::SetOnGetMenuContent(FOnGetContent InOnGetMenuContent)
{
	OnGetMenuContent = InOnGetMenuContent;
}

void SAvaMultiComboButton::SetButtonContentPadding(FMargin InPadding)
{
	check(ButtonPtr);
	ButtonPtr->SetContentPadding(InPadding);
}

void SAvaMultiComboButton::SetHasDownArrow(bool InHasArrowDown)
{
...
FReply SAvaMultiComboButton::OnMouseButtonDown(const FGeometry& InGeometry, const FPointerEvent& InMouseEvent)
{
	...
	return SMenuAnchor::OnMouseButtonDown(InGeometry, InMouseEvent);
}
