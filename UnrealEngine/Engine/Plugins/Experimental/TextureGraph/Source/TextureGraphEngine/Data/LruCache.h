...
/// Forked from TLruCache ["Core/Public/Containers/LruCache.h"]
...
public:
	 ...
	 */
	LruCache(int32 InMaxNumElements)
		: LeastRecent(nullptr)
		, MostRecent(nullptr)
...
public:
	...
	{
		check(MaxNumElements > 0 && "Cannot add values to zero size LruCache");
...
public:
	...
	public:
		...
		FORCEINLINE TBaseIterator(const LruCache& Cache)
			: CurrentEntry(Cache.LeastRecent)
		{ }
	...
	protected:
		...
		{
			check(CurrentEntry != nullptr);
			CurrentEntry = CurrentEntry->MoreRecent;
		}
	...
	public:
		...
		FORCEINLINE TConstIterator(const LruCache& Cache)
	...
	public:
		...
		FORCEINLINE TIterator(LruCache& InCache)
...
protected:
	 ...
	 */
	FORCEINLINE ValueType Remove(FCacheEntry* Entry)
	{
		if (Entry == nullptr)
		{
			return ValueType();
		...
		ValueType ReturnVal = Entry->Value;
