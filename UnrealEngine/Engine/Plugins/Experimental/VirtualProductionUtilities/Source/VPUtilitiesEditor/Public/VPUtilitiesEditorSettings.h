...
public:
	...
	UPROPERTY(EditAnywhere, config, Category = "Legacy Virtual Scouting", meta = (DisplayName = "Virtual Scouting User Interface"))
	TSoftClassPtr<UEditorUtilityWidget> VirtualScoutingUI;
	
	/** Speed when flying in VR*/
	UPROPERTY(EditAnywhere, config, Category = "Legacy Virtual Scouting", meta = (ClampMin = "0.0", ClampMax = "1.0", DisplayName = "Virtual Scouting Flight Speed"))
	float FlightSpeed = 0.5f;
	
	/** Speed when using grip nav in VR */
	UPROPERTY(EditAnywhere, config, Category = "Legacy Virtual Scouting", meta = (ClampMin = "0.0", ClampMax = "1.0", DisplayName = "Virtual Scouting Grip Nav Speed"))
	float GripNavSpeed = 0.25f;
	
	/** Whether to use the metric system or imperial for measurements */
	UPROPERTY(EditAnywhere, config, Category = "Legacy Virtual Scouting", meta = (DisplayName = "Show Measurements In Metric Units"))
	bool bUseMetric = false;
	
	/** Whether to enable or disable the transform gizmo */
	UPROPERTY(EditAnywhere, config, Category = "Legacy Virtual Scouting", meta = (DisplayName = "Enable Transform Gizmo In VR"))
	...
	UPROPERTY(EditAnywhere, config, Category = "Legacy Virtual Scouting", meta = (DisplayName = "Use Grip Inertia Damping"))
	bool bUseGripInertiaDamping = true;
	
	/** Damping applied to inertia */
	UPROPERTY(EditAnywhere, config, Category = "Legacy Virtual Scouting", meta = (ClampMin = "0.0", ClampMax = "1.0", DisplayName = "Inertia Damping"))
	float InertiaDamping = 0.95f;

	/** Whether the helper system on the controllers is enabled */
	UPROPERTY(EditAnywhere, config, Category = "Legacy Virtual Scouting", meta = (DisplayName = "Helper System Enabled "))
