...
#include UE_INLINE_GENERATED_CPP_BY_NAME(ChunkedStructBuffer)

void FChunkedStructBuffer::Reset()
{
...
void FChunkedStructBuffer::Compact()
{
...
void FChunkedStructBuffer::Clear()
{
	Reset();
	Compact();
}

void FChunkedStructBuffer::AllocateNewChunk()
{
	check(ChunkSize > 0);
...
void FChunkedStructBuffer::AddStructReferencedObjects(class FReferenceCollector& Collector)
{
	for (FChunkHeader* Chunk = First; Chunk != nullptr; Chunk = Chunk->Next)
	{
