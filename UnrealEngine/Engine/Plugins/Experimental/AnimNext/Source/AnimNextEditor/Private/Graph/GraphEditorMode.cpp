...
FGraphEditorSummoner::FGraphEditorSummoner(TSharedPtr<FWorkflowCentricApplication> InHostingApp)
	: FDocumentTabFactoryForObjects<UEdGraph>(GraphTabs::Document, InHostingApp)
{
}
...
class FGraphDetailsTabSummoner : public FWorkflowTabFactory
{
public:
	FGraphDetailsTabSummoner(TSharedPtr<FAssetEditorToolkit> InHostingApp, FOnDetailsViewCreated InOnDetailsViewCreated)
		: FWorkflowTabFactory(GraphTabs::Details, InHostingApp)
	{
...
FGraphEditorMode::FGraphEditorMode(TSharedRef<FWorkflowCentricApplication> InHostingApp)
	: FApplicationMode(GraphModes::GraphEditor)
	, HostingAppPtr(InHostingApp)
{
	...
	TabFactories.RegisterFactory(MakeShared<FGraphDetailsTabSummoner>(GraphEditor, FOnDetailsViewCreated::CreateSP(&GraphEditor.Get(), &FGraphEditor::HandleDetailsViewCreated)));

	TabLayout = FTabManager::NewLayout("Standalone_AnimNextGraphEditor_Layout_v1.0")
		->AddArea
		(
			...
			(	
				...
				(
					...
					->AddTab(GraphTabs::Document, ETabState::ClosedTab)
				)
				->Split
				(
					FTabManager::NewStack()
					...
					->SetHideTabWell(false)
					->AddTab(GraphTabs::Details, ETabState::OpenedTab)
				)
			)
		);
