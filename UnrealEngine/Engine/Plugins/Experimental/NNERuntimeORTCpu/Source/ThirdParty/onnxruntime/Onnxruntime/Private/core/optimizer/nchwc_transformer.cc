...
void NchwcTransformerImpl::TransformConv(Node& node) {
  ...
  if (filters_it != filters_map->end()) {
    ...
    InlinedVector<float> reordered_filter(narrow<size_t>(reordered_filter_size)); // WITH_UE
  ...
  if ((conv_B_tensor_proto != nullptr) && (output_channels != nchwc_output_channels)) {
    ...
    if (biases_it != aligned_biases_.end()) {
      ...
      InlinedVector<float> aligned_bias(narrow<size_t>(nchwc_output_channels)); // WITH_UE
      ...
      nchwc_conv_B_tensor_proto.set_raw_data(aligned_bias.data(), narrow<size_t>(nchwc_output_channels) * sizeof(float)); // WITH_UE
...
void NchwcTransformerImpl::TransformBatchNormalization(Node& node) {
  ...
  InlinedVector<float> padded_buffer(narrow<size_t>(nchwc_channels)); // WITH_UE
  ...
  nchwc_conv_W_tensor_proto.set_raw_data(padded_buffer.data(), narrow<size_t>(nchwc_channels) * sizeof(float)); // WITH_UE
  ...
  nchwc_conv_B_tensor_proto.set_raw_data(padded_buffer.data(), narrow<size_t>(nchwc_channels) * sizeof(float)); // WITH_UE
