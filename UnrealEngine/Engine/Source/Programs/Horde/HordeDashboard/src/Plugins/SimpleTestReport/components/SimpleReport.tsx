...
import { HistoryItem, LogLevel, SimpleReport } from '../models/SimpleReport';
...
const getStyling = () => {
   ...
   const styles = _styles ?? mergeStyleSets({
      ...
      historyList: {
         ...
         borderColor: "#888",
         borderStyle: "solid",
         backgroundColor: dashboard.darktheme ? "#111" : "#FFF",
         padding: "3px",
         overflow: "auto"
      },
      jobDetailsItem: {
   ...
   const stateStyles = _stateStyles ?? mergeStyleSets({
...
export const TestReportView: React.FC<{ data: SimpleReport, query: URLSearchParams }> = (props) => {
   ...
   const { styles, stateStyles } = getStyling();
   ...
   return (
      <Stack className={styles.container} styles={{ root: { backgroundColor: modeColors.backend, paddingLeft: 24, paddingTop: 12, paddingRight: 12 } }}>
