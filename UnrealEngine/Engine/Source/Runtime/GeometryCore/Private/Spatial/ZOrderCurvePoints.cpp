...
struct TIndexSpatialTree
{
	...
	void Init(TArrayView<const PointT> Points, BoxT Bounds, int32 MaxTreeDepth)
	{
		...
		auto PlaceInMaxDepthBucket = [this](int32 PtIdx, const PointT& Pt, NodeT& Node, const PointT& Center)
		{
		...
		for (int32 PtIdx = 0; PtIdx < Points.Num(); PtIdx++)
		{
			...
			while (true)
			{
				...
				{
					PlaceInMaxDepthBucket(PtIdx, Pt, Nodes[CurNodeIdx], CurCenter);
					break;
				}
					...
					else // didn't have room for the point: switch node to instead store indices to new children, and propagate old children down
					{
						...
						if (CurDepth + 1 >= MaxDepth)
						{
							...
							for (int32 SubIdx = 0; SubIdx < NodeT::NumPerLeaf; SubIdx++)
							{
								...
								PlaceInMaxDepthBucket(OldPtIdx, OldPt, NewChildNode, ChildCenter);
							}
						}
						else
						{
	...
	// Add indices of all points in the quadtree to the PointOrder array, following a z order curve when extracting them.
...
} // end of local helper function namespace
