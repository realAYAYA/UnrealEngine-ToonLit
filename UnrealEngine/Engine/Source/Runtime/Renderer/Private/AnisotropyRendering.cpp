...
static bool IsAnisotropyPassCompatible(const EShaderPlatform Platform, FMaterialShaderParameters MaterialParameters)
{
	return 
		...
		MaterialParameters.ShadingModels.HasAnyShadingModel({ MSM_DefaultLit, MSM_ClearCoat/* Change-begin*/, MSM_ToonHair/* Change-end*/ });
}
...
static bool ShouldDraw(const FMaterial& Material, bool bMaterialUsesAnisotropy)
{
	...
	return (bMaterialUsesAnisotropy && bIsNotTranslucent && Material.GetShadingModels().HasAnyShadingModel({ MSM_DefaultLit, MSM_ClearCoat/* Change-begin*/, MSM_ToonHair/* Change-end*/ }));
}
