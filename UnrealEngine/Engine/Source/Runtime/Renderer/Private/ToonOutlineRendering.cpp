...
void FSceneRenderer::RenderToonOutlinePass(FRDGBuilder& GraphBuilder, FRDGTextureRef SceneColorTexture)
{
	for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ++ViewIndex)
	{
		...
		{
			UE_LOG(LogShaders, Log, TEXT("FSceneRenderer::RenderToonOutlinePass() - View.Family->Scene is NULL!"));
			continue;
		}
		...
		GraphBuilder.AddPass(
			...
			{
				RHICmdList.SetViewport(ViewportRect.Min.X, ViewportRect.Min.Y, 0.0f, ViewportRect.Max.X,
				                       ViewportRect.Max.Y, 1.0f);
