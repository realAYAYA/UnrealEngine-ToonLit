...
{
	uint32 Key[4 * (AES256_ROUND_COUNT + 1)];
};
...
static const uint8 AesISbox[256] =
{
...
static const uint32 AesTbox[4][256] =
{
	{
...
static const uint32 AesITbox[4][256] =
{
	{
...
static inline uint32 AesEncryptMix(uint32 x)
{
	return ((uint32)AesSbox[(uint8)(x >>  0)] <<  0)
		 ^ ((uint32)AesSbox[(uint8)(x >>  8)] <<  8)
		 ^ ((uint32)AesSbox[(uint8)(x >> 16)] << 16)
		 ^ ((uint32)AesSbox[(uint8)(x >> 24)] << 24);
}

static inline uint32 AesDecryptMix(uint32 x)
{
	return (AesITbox[0][AesSbox[(uint8)(x >>  0)]])
		 ^ (AesITbox[1][AesSbox[(uint8)(x >>  8)]])
		 ^ (AesITbox[2][AesSbox[(uint8)(x >> 16)]])
...
static inline void AesEncryptExpand(FAesExpandedKey* EncryptKey, const uint8* Key)
{
	...
	EKey[0] = FPlatformMemory::ReadUnaligned<uint32>(Key +  0);
	EKey[1] = FPlatformMemory::ReadUnaligned<uint32>(Key +  4);
	EKey[2] = FPlatformMemory::ReadUnaligned<uint32>(Key +  8);
	EKey[3] = FPlatformMemory::ReadUnaligned<uint32>(Key + 12);
	EKey[4] = FPlatformMemory::ReadUnaligned<uint32>(Key + 16);
	EKey[5] = FPlatformMemory::ReadUnaligned<uint32>(Key + 20);
...
static inline void AesDecryptExpand(FAesExpandedKey* DecryptKey, const uint8* Key)
{
	...
	const uint32* EKey = EncryptKey.Key + (4 * AES256_ROUND_COUNT);
	uint32* DKey = DecryptKey->Key;
...
#define AES_ENC_LAST(a, b, c, d) (      \
	...
	for (uint64 Offset = 0; Offset < NumBytes; Offset += 16)
	{
		...
		uint32 s0 = FPlatformMemory::ReadUnaligned<uint32>(Block +  0) ^ EKey[0];
		uint32 s1 = FPlatformMemory::ReadUnaligned<uint32>(Block +  4) ^ EKey[1];
		uint32 s2 = FPlatformMemory::ReadUnaligned<uint32>(Block +  8) ^ EKey[2];
		uint32 s3 = FPlatformMemory::ReadUnaligned<uint32>(Block + 12) ^ EKey[3];

		for (int Round = 1; Round < AES256_ROUND_COUNT; Round++)
		{
			uint32 t0 = AES_ENC(s0, s1, s2, s3);
			uint32 t1 = AES_ENC(s1, s2, s3, s0);
			uint32 t2 = AES_ENC(s2, s3, s0, s1);
			uint32 t3 = AES_ENC(s3, s0, s1, s2);

			s0 = t0 ^ EKey[4 * Round + 0];
			s1 = t1 ^ EKey[4 * Round + 1];
			s2 = t2 ^ EKey[4 * Round + 2];
			s3 = t3 ^ EKey[4 * Round + 3];
		}

		uint32 t0 = AES_ENC_LAST(s0, s1, s2, s3);
		uint32 t1 = AES_ENC_LAST(s1, s2, s3, s0);
		uint32 t2 = AES_ENC_LAST(s2, s3, s0, s1);
		uint32 t3 = AES_ENC_LAST(s3, s0, s1, s2);

		s0 = t0 ^ EKey[4 * AES256_ROUND_COUNT + 0];
		s1 = t1 ^ EKey[4 * AES256_ROUND_COUNT + 1];
		s2 = t2 ^ EKey[4 * AES256_ROUND_COUNT + 2];
		s3 = t3 ^ EKey[4 * AES256_ROUND_COUNT + 3];

		FPlatformMemory::WriteUnaligned<uint32>(Block +  0, s0);
		FPlatformMemory::WriteUnaligned<uint32>(Block +  4, s1);
		FPlatformMemory::WriteUnaligned<uint32>(Block +  8, s2);
		FPlatformMemory::WriteUnaligned<uint32>(Block + 12, s3);
	}
...
#define AES_DEC_LAST(a, b, c, d) (       \
	...
	AesDecryptExpand(&DecryptKey, Key);
	...
	for (uint64 Offset = 0; Offset < NumBytes; Offset += 16)
	{
		...
		uint32 s0 = FPlatformMemory::ReadUnaligned<uint32>(Block +  0) ^ DKey[0];
		uint32 s1 = FPlatformMemory::ReadUnaligned<uint32>(Block +  4) ^ DKey[1];
		uint32 s2 = FPlatformMemory::ReadUnaligned<uint32>(Block +  8) ^ DKey[2];
		uint32 s3 = FPlatformMemory::ReadUnaligned<uint32>(Block + 12) ^ DKey[3];

		for (int Round = 1; Round < AES256_ROUND_COUNT; Round++)
		{
			uint32 t0 = AES_DEC(s0, s3, s2, s1);
			uint32 t1 = AES_DEC(s1, s0, s3, s2);
			uint32 t2 = AES_DEC(s2, s1, s0, s3);
			uint32 t3 = AES_DEC(s3, s2, s1, s0);
		...
		uint32 t0 = AES_DEC_LAST(s0, s3, s2, s1);
		uint32 t1 = AES_DEC_LAST(s1, s0, s3, s2);
		uint32 t2 = AES_DEC_LAST(s2, s1, s0, s3);
		uint32 t3 = AES_DEC_LAST(s3, s2, s1, s0);
...
#define AES_INIT_ENC_X86(ctx, x, y, rcon)         \
{                                                 \
    __m128i z, t;                                 \
    ctx = y;                                      \
...
static inline void AesEncryptX86(const void* Key, void* Contents, uint64 NumBytes)
{
	...
	__m128i EKey[AES256_ROUND_COUNT + 1];
	EKey[0] = x;
	AES_INIT_ENC_X86(EKey[ 1], x, y, 0x01);
	AES_INIT_ENC_X86(EKey[ 2], y, x, 0x00);
	AES_INIT_ENC_X86(EKey[ 3], x, y, 0x02);
	AES_INIT_ENC_X86(EKey[ 4], y, x, 0x00);
	AES_INIT_ENC_X86(EKey[ 5], x, y, 0x04);
	AES_INIT_ENC_X86(EKey[ 6], y, x, 0x00);
	AES_INIT_ENC_X86(EKey[ 7], x, y, 0x08);
	AES_INIT_ENC_X86(EKey[ 8], y, x, 0x00);
	AES_INIT_ENC_X86(EKey[ 9], x, y, 0x10);
	AES_INIT_ENC_X86(EKey[10], y, x, 0x00);
	AES_INIT_ENC_X86(EKey[11], x, y, 0x20);
	AES_INIT_ENC_X86(EKey[12], y, x, 0x00);
	AES_INIT_ENC_X86(EKey[13], x, y, 0x40);
	EKey[14] = x;
	...
	while (NumBytes >= 32)
	{
		...
		Block0 = _mm_xor_si128(Block0, EKey[0]);
		Block1 = _mm_xor_si128(Block1, EKey[0]);
		Block0 = _mm_aesenc_si128(Block0, EKey[1]);
		Block1 = _mm_aesenc_si128(Block1, EKey[1]);
		Block0 = _mm_aesenc_si128(Block0, EKey[2]);
		Block1 = _mm_aesenc_si128(Block1, EKey[2]);
		Block0 = _mm_aesenc_si128(Block0, EKey[3]);
		Block1 = _mm_aesenc_si128(Block1, EKey[3]);
		Block0 = _mm_aesenc_si128(Block0, EKey[4]);
		Block1 = _mm_aesenc_si128(Block1, EKey[4]);
		Block0 = _mm_aesenc_si128(Block0, EKey[5]);
		Block1 = _mm_aesenc_si128(Block1, EKey[5]);
		Block0 = _mm_aesenc_si128(Block0, EKey[6]);
		Block1 = _mm_aesenc_si128(Block1, EKey[6]);
		Block0 = _mm_aesenc_si128(Block0, EKey[7]);
		Block1 = _mm_aesenc_si128(Block1, EKey[7]);
		Block0 = _mm_aesenc_si128(Block0, EKey[8]);
		Block1 = _mm_aesenc_si128(Block1, EKey[8]);
		Block0 = _mm_aesenc_si128(Block0, EKey[9]);
		Block1 = _mm_aesenc_si128(Block1, EKey[9]);
		Block0 = _mm_aesenc_si128(Block0, EKey[10]);
		Block1 = _mm_aesenc_si128(Block1, EKey[10]);
		Block0 = _mm_aesenc_si128(Block0, EKey[11]);
		Block1 = _mm_aesenc_si128(Block1, EKey[11]);
		Block0 = _mm_aesenc_si128(Block0, EKey[12]);
		Block1 = _mm_aesenc_si128(Block1, EKey[12]);
		Block0 = _mm_aesenc_si128(Block0, EKey[13]);
		Block1 = _mm_aesenc_si128(Block1, EKey[13]);
		Block0 = _mm_aesenclast_si128(Block0, EKey[14]);
		Block1 = _mm_aesenclast_si128(Block1, EKey[14]);
	...
	if (NumBytes != 0)
	{
		...
		Block = _mm_xor_si128(Block, EKey[0]);
		Block = _mm_aesenc_si128(Block, EKey[1]);
		Block = _mm_aesenc_si128(Block, EKey[2]);
		Block = _mm_aesenc_si128(Block, EKey[3]);
		Block = _mm_aesenc_si128(Block, EKey[4]);
		Block = _mm_aesenc_si128(Block, EKey[5]);
		Block = _mm_aesenc_si128(Block, EKey[6]);
		Block = _mm_aesenc_si128(Block, EKey[7]);
		Block = _mm_aesenc_si128(Block, EKey[8]);
		Block = _mm_aesenc_si128(Block, EKey[9]);
		Block = _mm_aesenc_si128(Block, EKey[10]);
		Block = _mm_aesenc_si128(Block, EKey[11]);
		Block = _mm_aesenc_si128(Block, EKey[12]);
		Block = _mm_aesenc_si128(Block, EKey[13]);
		Block = _mm_aesenclast_si128(Block, EKey[14]);
...
static inline void AesEncryptARMV8(const uint8* Key, uint8* Contents, uint64 NumBytes)
{
	...
	for (uint64 Offset = 0; Offset < NumBytes; Offset += 16)
	{
		...
		uint8x16_t Block = vld1q_u8(Data);
		Block = vaesmcq_u8(vaeseq_u8(Block, EKey[0]));
		Block = vaesmcq_u8(vaeseq_u8(Block, EKey[1]));
		Block = vaesmcq_u8(vaeseq_u8(Block, EKey[2]));
		Block = vaesmcq_u8(vaeseq_u8(Block, EKey[3]));
		Block = vaesmcq_u8(vaeseq_u8(Block, EKey[4]));
		Block = vaesmcq_u8(vaeseq_u8(Block, EKey[5]));
		Block = vaesmcq_u8(vaeseq_u8(Block, EKey[6]));
		Block = vaesmcq_u8(vaeseq_u8(Block, EKey[7]));
		Block = vaesmcq_u8(vaeseq_u8(Block, EKey[8]));
		Block = vaesmcq_u8(vaeseq_u8(Block, EKey[9]));
		Block = vaesmcq_u8(vaeseq_u8(Block, EKey[10]));
		Block = vaesmcq_u8(vaeseq_u8(Block, EKey[11]));
		Block = vaesmcq_u8(vaeseq_u8(Block, EKey[12]));
		Block = vaeseq_u8(Block, EKey[13]);
		Block = veorq_u8(Block, EKey[14]);
		vst1q_u8(Data, Block);
	}
}
...
static inline void AesDecryptARMV8(const uint8* Key, uint8* Contents, uint64 NumBytes)
{
	...
	for (uint64 Offset = 0; Offset < NumBytes; Offset += 16)
	{
		...
		uint8x16_t Block = vld1q_u8(Data);
		Block = vaesimcq_u8(vaesdq_u8(Block, DKey[0]));
		Block = vaesimcq_u8(vaesdq_u8(Block, DKey[1]));
		Block = vaesimcq_u8(vaesdq_u8(Block, DKey[2]));
		Block = vaesimcq_u8(vaesdq_u8(Block, DKey[3]));
		Block = vaesimcq_u8(vaesdq_u8(Block, DKey[4]));
		Block = vaesimcq_u8(vaesdq_u8(Block, DKey[5]));
		Block = vaesimcq_u8(vaesdq_u8(Block, DKey[6]));
		Block = vaesimcq_u8(vaesdq_u8(Block, DKey[7]));
		Block = vaesimcq_u8(vaesdq_u8(Block, DKey[8]));
		Block = vaesimcq_u8(vaesdq_u8(Block, DKey[9]));
		Block = vaesimcq_u8(vaesdq_u8(Block, DKey[10]));
		Block = vaesimcq_u8(vaesdq_u8(Block, DKey[11]));
		Block = vaesimcq_u8(vaesdq_u8(Block, DKey[12]));
		Block = vaesdq_u8(Block, DKey[13]);
		Block = veorq_u8(Block, DKey[14]);
		vst1q_u8(Data, Block);
	}
}
