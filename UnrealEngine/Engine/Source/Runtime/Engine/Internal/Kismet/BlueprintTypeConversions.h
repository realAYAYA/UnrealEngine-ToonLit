...
namespace UE::Kismet::BlueprintTypeConversions
{
	...
	class ENGINE_API FStructConversionTable
	{
	public:
		static const FStructConversionTable& Get();

		TOptional<UE::Kismet::BlueprintTypeConversions::ConversionFunctionPairT>
...
public:
	...
	static TArray<int> ConvertArrayType(const TArray<int>& InArray);
	DECLARE_FUNCTION(execConvertArrayType);

	UFUNCTION(CustomThunk, meta = (BlueprintInternalUseOnly = "true"))
	static TSet<int> ConvertSetType(const TSet<int>& InSet);
