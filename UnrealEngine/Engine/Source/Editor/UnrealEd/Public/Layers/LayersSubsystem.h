...
public:
	...
	*/
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = Layers)
	bool RemoveActorsFromLayer(const TArray< AActor* >& Actors, const FName& LayerName, const bool bUpdateStats = true);
	/**
	 * Removes the actors from the specified layer.
	 *
	 * @param	Actors			The actors to remove from the provided layer
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = Layers)
	bool RemoveActorsFromLayers(const TArray< AActor* >& Actors, const TArray< FName >& LayerNames, const bool bUpdateStats = true);
	/**
	 * Remove the actors to the named layers
	 *
	 * @param	Actors		The actors to remove to the named layers
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = Layers)
	bool SelectActorsInLayer(const FName& LayerName, const bool bSelect, const bool bNotify, const bool bSelectEvenIfHidden = false);
	/**
	 * Selects/de-selects actors belonging to the named layer.
	 *
	 * @param	LayerName						A valid layer name.
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = Layers)
	bool SelectActorsInLayers(const TArray< FName >& LayerNames, const bool bSelect, const bool bNotify, const bool bSelectEvenIfHidden = false);
	/**
	 * Selects/de-selects actors belonging to the named layers.
	 *
	 * @param	LayerNames						A valid list of layer names.
	 ...
	 *  @param	Filter	[optional]	Actor that don't pass the specified filter restrictions won't be selected.
	 */
	void AppendActorsFromLayer(const FName& LayerName, TArray< AActor* >& InOutActors, const TSharedPtr< ActorFilter >& Filter) const;
	/**
	 *	Appends all the actors associated with the specified layer.
	 *
	 *	@param	LayerName			The layer to find actors for.
	 ...
	 *  @param	Filter	[optional]	Actor that don't pass the specified filter restrictions won't be selected.
	 */
	void AppendActorsFromLayers(const TArray< FName >& LayerNames, TArray< AActor* >& InOutActors, const TSharedPtr< ActorFilter >& Filter) const;
	/**
	 *	Appends all the actors associated with ANY of the specified layers.
	 *
	 *	@param	LayerNames			The layers to find actors for.
	 ...
	 *	@return						The list to assign the found actors to.
	 */
	TArray< AActor* > GetActorsFromLayer(const FName& LayerName, const TSharedPtr< ActorFilter >& Filter) const;
	/**
	 ...
	 *	@return						The list to assign the found actors to.
	 */
	TArray< AActor* > GetActorsFromLayers(const TArray< FName >& LayerNames, const TSharedPtr< ActorFilter >& Filter) const;
...
private:
	...
	void RemoveActorFromStats(ULayer* Layer, AActor* Actor);
	 ...
	 */
	ULayer* EnsureLayerExists(const FName& LayerName);
