...
namespace jpge {
	...
	enum { kCONST_BITS = 13, kROW_BITS = 2 };
  ...
  s0 = t10 + t11; s1 = t7 + u1 + u4; s3 = t6 + u2 + u3; s4 = t10 - t11; s5 = t5 + u2 + u4; s7 = t4 + u1 + u3;
	...
	static void DCT2D(int32* p)
	{
		...
		for (c = 7; c >= 0; c--, q += 8)
		{
			...
			q[0] = left_shifti(s0, kROW_BITS); q[1] = DCT_DESCALE(s1, kCONST_BITS - kROW_BITS); q[2] = DCT_DESCALE(s2, kCONST_BITS - kROW_BITS); q[3] = DCT_DESCALE(s3, kCONST_BITS - kROW_BITS);
			q[4] = left_shifti(s4, kROW_BITS); q[5] = DCT_DESCALE(s5, kCONST_BITS - kROW_BITS); q[6] = DCT_DESCALE(s6, kCONST_BITS - kROW_BITS); q[7] = DCT_DESCALE(s7, kCONST_BITS - kROW_BITS);
		}
		for (q = p, c = 7; c >= 0; c--, q++)
		{
			int32 s0 = q[0 * 8], s1 = q[1 * 8], s2 = q[2 * 8], s3 = q[3 * 8], s4 = q[4 * 8], s5 = q[5 * 8], s6 = q[6 * 8], s7 = q[7 * 8];
			DCT1D(s0, s1, s2, s3, s4, s5, s6, s7);
			q[0 * 8] = DCT_DESCALE(s0, kROW_BITS + 3); q[1 * 8] = DCT_DESCALE(s1, kCONST_BITS + kROW_BITS + 3); q[2 * 8] = DCT_DESCALE(s2, kCONST_BITS + kROW_BITS + 3); q[3 * 8] = DCT_DESCALE(s3, kCONST_BITS + kROW_BITS + 3);
			q[4 * 8] = DCT_DESCALE(s4, kROW_BITS + 3); q[5 * 8] = DCT_DESCALE(s5, kCONST_BITS + kROW_BITS + 3); q[6 * 8] = DCT_DESCALE(s6, kCONST_BITS + kROW_BITS + 3); q[7 * 8] = DCT_DESCALE(s7, kCONST_BITS + kROW_BITS + 3);
		}
	}
...
#define JPGE_PUT_BYTE(c) { *m_pOut_buf++ = (c); if (--m_out_buf_left == 0) flush_output_buffer(); }
	...
	bool jpeg_encoder::init(output_stream* pStream, int64_t width, int64_t height, int64_t src_channels, const params& comp_params)
	{
		...
		if (width >= 65536 || height >= 65536) return false;
		m_pStream = pStream;
		m_params = comp_params;
		return jpg_open((int)width, (int)height, (int)src_channels);
	}
