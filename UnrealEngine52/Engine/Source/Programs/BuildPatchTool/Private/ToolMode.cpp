...
namespace BuildPatchTool
{
	...
	public:
		...
		virtual EReturnCode Execute() override
		{
			...
			UE_LOG(LogBuildPatchTool, Display, TEXT("-help can be added with any mode selection to get extended information."));
			UE_LOG(LogBuildPatchTool, Display, TEXT("Supported modes are:"));
			UE_LOG(LogBuildPatchTool, Display, TEXT("  -mode=PatchGeneration      Mode that generates patch data for the a new build."));
			UE_LOG(LogBuildPatchTool, Display, TEXT("  -mode=ChunkDeltaOptimise   Mode that produces a more optimal chunked patch between two specific builds."));
			UE_LOG(LogBuildPatchTool, Display, TEXT("  -mode=Compactify           Mode that can clean up unneeded patch data from a given cloud directory with redundant data."));
			UE_LOG(LogBuildPatchTool, Display, TEXT("  -mode=Enumeration          Mode that outputs the paths to referenced patch data given a single manifest."));
			UE_LOG(LogBuildPatchTool, Display, TEXT("  -mode=MergeManifests       Mode that can combine two manifest files to create a new one, primarily used to create hotfixes."));
			UE_LOG(LogBuildPatchTool, Display, TEXT("  -mode=DiffManifests        Mode that can diff two manifests and outputs what chunks would need to be downloaded and some stats."));
			UE_LOG(LogBuildPatchTool, Display, TEXT("  -mode=PackageChunks        Mode that packages data required for an installation into larger files which can be used as local sources for build patch installers."));
			UE_LOG(LogBuildPatchTool, Display, TEXT("  -mode=VerifyChunks         Mode that allows you to verify the integrity of patch data. It will load chunk or chunkdb files to verify they are not corrupt."));
#if !UE_BUILD_SHIPPING
			UE_LOG(LogBuildPatchTool, Display, TEXT("  -mode=AutomationTests      Mode that will run automation testing."));
	...
	IToolModeRef FToolModeFactory::Create(IBuildPatchServicesModule& BpsInterface)
	{
		...
		if (FParse::Value(FCommandLine::Get(), TEXT("mode="), ToolModeValue))
		{
			...
			else if (ToolModeValue == TEXT("chunkdeltaoptimise"))
			{
				return FChunkDeltaOptimiseToolModeFactory::Create(BpsInterface);
			}
