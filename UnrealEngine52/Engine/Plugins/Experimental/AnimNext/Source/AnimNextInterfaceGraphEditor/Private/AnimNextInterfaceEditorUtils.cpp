...
void FUtils::GetAllGraphNames(const UAnimNextInterfaceGraph_EditorData* InEditorData, TSet<FName>& OutNames)
{
...
FName FUtils::ValidateName(const UAnimNextInterfaceGraph_EditorData* InEditorData, const FString& InName)
{
	struct FNameValidator : public INameValidatorInterface
	{
		FNameValidator(const UAnimNextInterfaceGraph_EditorData* InEditorData)
		...
		virtual EValidatorResult IsValid (const FName& Name, bool bOriginal = false) override
		{
			...
			else
			{
				...
				if(!Names.Contains(Name))
				{
					...
					if (UObject* ExistingObject = StaticFindObject(/*Class=*/ nullptr, const_cast<UAnimNextInterfaceGraph_EditorData*>(EditorData), *Name.ToString(), true))
					{
		...
		const UAnimNextInterfaceGraph_EditorData* EditorData;
	};
