...
//        new_tets[i] = std::array<MeshTet, 2>({{new_ts[0], new_ts[1]}});
        ...
        for (auto &_t: new_ts) {
            qs.emplace_back(
                    get_quality(tet_vertices[_t[0]], tet_vertices[_t[1]], tet_vertices[_t[2]], tet_vertices[_t[3]]));
        }
...
//        new_ts.push_back(t);
        ...
        std::vector<Scalar> qs;
        for (auto &_t:new_ts) {
            qs.push_back(get_quality(tet_vertices[_t[0]], tet_vertices[_t[1]], tet_vertices[_t[2]], tet_vertices[_t[3]]));
        }
        for (int j = 0; j < 2; j++) {
